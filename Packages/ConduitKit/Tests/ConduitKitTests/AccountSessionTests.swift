...
struct AccountSessionTests {
    ...
    @Test("password reset uses the Conduit deep-link and restores the returned session")
    func passwordResetDeepLink() async throws {
        ...
        #expect(await recorder.redirect()?.absoluteString == "conduit://auth/callback")

        try await controller.completePasswordReset(
            callbackURL: URL(string: "conduit://auth/callback#access_token=token")!,
