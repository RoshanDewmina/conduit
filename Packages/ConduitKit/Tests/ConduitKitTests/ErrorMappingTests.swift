...
struct ErrorMappingTests {
    ...
    // MARK: - ConduitError pass-through

    @Test("ConduitError.timeout passes through unchanged")
    func conduitErrorPassthrough() {
        #expect(SSHSession.map(error: ConduitError.timeout, host: "h") == .timeout)
    }

    @Test("ConduitError.notConnected passes through unchanged")
    func conduitNotConnectedPassthrough() {
        #expect(SSHSession.map(error: ConduitError.notConnected, host: "h") == .notConnected)
    }
