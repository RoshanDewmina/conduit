...
public extension View {
    ...
    func conduitMotion<Value: Equatable>(_ animation: Animation, value: Value) -> some View {
        modifier(ConduitMotionModifier(animation: animation, value: value))
    }
}

private struct ConduitMotionModifier<Value: Equatable>: ViewModifier {
    let animation: Animation
    let value: Value
    @Environment(\.accessibilityReduceMotion) private var reduceMotion

    func body(content: Content) -> some View {
        content.animation(ConduitMotion.resolved(animation, reduceMotion: reduceMotion), value: value)
    }
}

public struct ConduitPage<Content: View>: View {
...
public struct ConduitScreenHeader<Trailing: View>: View {
...
public enum ConduitAttentionTone {
    case attention
    case safe
    case working
...
public struct ConduitAttentionBand: View {
    private let eyebrow: String
    private let title: String
    private let detail: String
    private let tone: ConduitAttentionTone
    ...
    public init(
        ...
        title: String,
        detail: String,
        tone: ConduitAttentionTone = .attention,
        action: (() -> Void)? = nil
    ) {
...
public struct ConduitSectionLabel: View {
    private let title: String
    private let detail: String?
...
public extension View {
    func conduitSurfaceCard(padding: CGFloat = 14) -> some View {
        modifier(ConduitSurfaceCardModifier(padding: padding))
    }
}

private struct ConduitSurfaceCardModifier: ViewModifier {
    let padding: CGFloat
    @Environment(\.conduitTokens) private var t
