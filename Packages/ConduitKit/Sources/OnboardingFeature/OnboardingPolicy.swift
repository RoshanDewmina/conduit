...
/// Maps the onboarding "How cautious?" tier to a starter `~/.conduit/policy.yaml`
...
public enum OnboardingPolicy {
    ...
    static let pendingKey = "conduit.onboarding.pendingPolicyLevel"
    /// Set once the starter policy has been pushed + reloaded successfully.
    static let appliedKey = "conduit.onboarding.policyApplied"
...
public extension OnboardingCautionLevel {
    /// Starter `~/.conduit/policy.yaml` for this tier. Schema + semantics match
    ...
    var policyYAML: String {
        ...
        case .cautious:
            return """
            # Conduit starter policy — Cautious
        ...
        case .balanced:
            return """
            # Conduit starter policy — Balanced (recommended)
