...
/// random UUID, so conduitd registered/polled under one id while the relay
/// POSTed decisions under another → the decision landed under a session key
/// conduitd never polled → silent 120 s timeout-deny, and APNs approvals never
/// reached the device.
///
...
public enum DeviceIdentity {
    private static let storageKey = "dev.conduit.stableDeviceSessionID"
