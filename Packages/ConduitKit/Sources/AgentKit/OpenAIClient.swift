...
public actor OpenAIClient: AIClient {
                ...
                do {
                    ...
                    guard http.statusCode == 200 else {
                        throw http.statusCode == 429
                            ...
                            : ConduitError.providerUnavailable(name: "OpenAI", status: http.statusCode)
                    }
    ...
    public func complete(messages: [AIMessage], system: String?, maxTokens: Int) async throws -> String {
        ...
        guard let http = response as? HTTPURLResponse, http.statusCode == 200 else {
            throw ConduitError.providerUnavailable(name: "OpenAI", status: (response as? HTTPURLResponse)?.statusCode)
        }
