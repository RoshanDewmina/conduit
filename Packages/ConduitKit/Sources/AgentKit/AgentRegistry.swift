...
// Conduit drops the on-disk JSON config discovery (cmux loads ~/.config/cmux/cmux.json
// and project-local .cmux/cmux.json — those don't apply on iOS) and the local
// process detection (cmux uses pgrep against running processes; Conduit instead
...
/// Stable identifiers for agents whose resume command shape lives in Conduit
...
public struct AgentRegistration: Codable, Hashable, Sendable {
    ...
    //
    // Conduit ships these as defaults. They cover the agents Conduit users
...
/// matches against local processes via pgrep; Conduit uses the same rule
...
public struct AgentRegistry: Sendable {
    private static let logger = Logger(subsystem: "dev.conduit.mobile", category: "AgentRegistry")
    ...
    /// Default registry covering all built-in agents Conduit ships with.
