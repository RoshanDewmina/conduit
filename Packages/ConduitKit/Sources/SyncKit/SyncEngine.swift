...
public actor SyncEngine {
    ...
    private func mergedHost(remote: ConduitCore.Host, local: ConduitCore.Host) -> ConduitCore.Host {
        var merged = remote
    ...
    private func hasMeaningfulDifference(_ a: ConduitCore.Host, _ b: ConduitCore.Host) -> Bool {
        a.name != b.name || a.hostname != b.hostname || a.port != b.port || a.username != b.username
    }

    // MARK: - Record decoding

    private func hostFrom(wrapper: CKRecordWrapper) -> ConduitCore.Host? {
        #if os(iOS)
