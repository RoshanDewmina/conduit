...
///
/// Unknown fingerprint → fails with `ConduitError.hostKeyUnknown` so the UI
...
final class TOFUHostKeyValidator: NIOSSHClientServerAuthenticationDelegate, @unchecked Sendable {
    ...
    func validateHostKey(
        ...
        Task {
            ...
            case .unknown:
                validationCompletePromise.fail(ConduitError.hostKeyUnknown(fingerprint: fingerprint))
            case .mismatch(let expected, let actual):
                validationCompletePromise.fail(
                    ConduitError.hostKeyMismatch(expected: expected, actual: actual)
                )
            }
        }
    }
