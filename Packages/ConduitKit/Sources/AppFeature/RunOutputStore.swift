...
public final class RunOutputStore {
    ...
    public func register(runId: String, status: String = "running") {
        if runs[runId] == nil {
            runs[runId] = Run(runId: runId, chunks: [], status: status, exitCode: nil)
        }
    }

    public func appendOutput(_ params: RunOutputParams) {
        guard !params.runId.isEmpty else { return }
        var run = runs[params.runId] ?? Run(runId: params.runId, chunks: [], status: "running", exitCode: nil)
    ...
    public func updateStatus(_ params: RunStatusParams) {
        guard !params.runId.isEmpty else { return }
        var run = runs[params.runId] ?? Run(runId: params.runId, chunks: [], status: params.status, exitCode: nil)
