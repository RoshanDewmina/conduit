...
// LancerSessionAttributes.ContentState is a plain Codable struct — no
...
struct LiveActivityContentStateTests {
    ...
    func reconnectingRoundTrip() throws {
        let original = LancerSessionAttributes.ContentState(
            status: "reconnecting",
            pendingApprovals: 1,
            agentName: "Codex",
        ...
        let decoded = try JSONDecoder().decode(
            LancerSessionAttributes.ContentState.self,
            from: data
        )
    ...
    func suspendedRoundTrip() throws {
        let original = LancerSessionAttributes.ContentState(status: "suspended")
        let data = try JSONEncoder().encode(original)
        let decoded = try JSONDecoder().decode(
            LancerSessionAttributes.ContentState.self,
            from: data
    ...
    func pushBackendContentStateRoundTrip() throws {
        ...
        // types as LancerSessionAttributes.ContentState. This test encodes a
