...
struct SessionSnapshotRepositoryTests {
    ...
    func upsertAndRead() async throws {
        ...
        let host = LancerCore.Host(name: "test", hostname: "x", username: "u")
    ...
    func touchUpdatesTime() async throws {
        let db = try AppDatabase.inMemory()
        let host = LancerCore.Host(name: "t", hostname: "x", username: "u")
        try await HostRepository(db).upsert(host)
        let repo = SessionSnapshotRepository(db)
    ...
    func allRecentSorted() async throws {
        ...
        let h1 = LancerCore.Host(name: "old", hostname: "x", username: "u")
        let h2 = LancerCore.Host(name: "new", hostname: "x", username: "u")
