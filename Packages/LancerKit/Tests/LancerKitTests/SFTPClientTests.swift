...
struct SFTPClientTests {
    ...
    @Test func listDirectoryRoundTrip() async throws {
        guard let host = ProcessInfo.processInfo.environment["LANCER_SSH_HOST"] else {
            return  // skip in CI
        }
        guard
            let user = ProcessInfo.processInfo.environment["LANCER_SSH_USER"],
            let rawKey = ProcessInfo.processInfo.environment["LANCER_SSH_ED25519_PRIVATE_KEY_B64"],
        ...
        let fixtureName = "lancer-sftp-test-\(UUID().uuidString.prefix(8)).txt"
        let fixturePath = "./\(fixtureName)"
        let payload = Data("hello from lancer tests\n".utf8)
