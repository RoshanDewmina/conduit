...
struct AuthCredentialTests {
    ...
    @Test("map(error:) returns .authFailed for LancerError.authFailed pass-through")
    func authFailedPassthrough() {
        let err = LancerError.authFailed(reason: "bad password")
    ...
    @Test("map(error:) returns .timeout for LancerError.timeout pass-through")
    func timeoutPassthrough() {
        let mapped = SSHSession.map(error: LancerError.timeout, host: "h")
