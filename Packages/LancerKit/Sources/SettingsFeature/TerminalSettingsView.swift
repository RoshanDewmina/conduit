...
private struct ShellIntegrationDiagnosticsRow: View {
    @AppStorage("lancerShellDetected")  private var shellDetected: String = ""
    @AppStorage("lancerMarkersActive")  private var markersActive: Bool = false
    @AppStorage("lancerLastMarkerTime") private var lastMarkerTime: Double = 0
    @Environment(\.lancerTokens) private var t
...
private struct DebugProBypassToggle: View {
    @State private var isOn: Bool = UserDefaults.standard.bool(forKey: "lancerDebugProBypass")
    @Environment(\.lancerTokens) private var t
        ...
        VStack(alignment: .leading, spacing: 4) {
            Toggle("Unlock all features (debug)", isOn: $isOn)
                ...
                .onChange(of: isOn) { _, newValue in
                    UserDefaults.standard.set(newValue, forKey: "lancerDebugProBypass")
                }
