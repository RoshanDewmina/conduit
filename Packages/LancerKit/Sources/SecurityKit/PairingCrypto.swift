...
///
/// This is mirrored from the Helm protocol so a future Lancer desktop
...
public enum PairingCrypto {
    ...
    private static let frameAAD = Data("lancer-frame-v1".utf8)
    ...
    public static func deriveSessionKey(
        ...
        do {
            ...
            throw LancerError.keyDecodeFailed(reason: "peer public key")
        }
        let shared = try privateKey.sharedSecretFromKeyAgreement(with: peer)
        let saltSeed = SHA256.hash(data: Data("lancer-pairing:\(helperID)".utf8))
        return shared.hkdfDerivedSymmetricKey(
            using: SHA256.self,
            salt: Data(saltSeed),
            sharedInfo: Data("lancer-v1:\(helperPublicKeyBase64URL):\(appPublicKeyBase64URL)".utf8),
            outputByteCount: 32
        )
    }
    ...
    public static func decrypt(_ frame: EncryptedFrame, using key: SymmetricKey) throws -> Data {
        guard frame.version == frameVersion else {
            throw LancerError.invalidResponse(detail: "frame version \(frame.version)")
        }
        let nonceData = try Base64URL.decode(frame.nonce)
        guard nonceData.count == nonceByteCount else {
            throw LancerError.invalidResponse(detail: "bad nonce length")
        }
    ...
    private static func randomBytes(count: Int) throws -> Data {
        ...
        guard status == errSecSuccess else {
            throw LancerError.unknown(detail: "SecRandomCopyBytes \(status)")
        }
        return Data(bytes)
    }
}
...
public enum Base64URL {
    ...
    public static func decode(_ string: String) throws -> Data {
        ...
        guard let data = Data(base64Encoded: s) else {
            throw LancerError.invalidResponse(detail: "bad base64url")
        }
        return data
    }
}
