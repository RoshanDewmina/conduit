...
public actor KeyStore {
    ...
    public init(service: String = "dev.lancer.mobile.sshkeys", inMemory: Bool = false) {
    ...
    public func importPrivateKey(tag: String, keyString: String, comment: String? = nil) async throws -> PublicKeyInfo {
        ...
        guard !normalized.isEmpty else {
            throw LancerError.keyDecodeFailed(reason: "Key text is empty.")
        }
        ...
        if normalized.contains("BEGIN OPENSSH PRIVATE KEY") {
            ...
            case .ecdsaP256, .ecdsaP384, .ecdsaP521:
                throw LancerError.keyDecodeFailed(
                    reason: "OpenSSH ECDSA private keys are unsupported. Import a PEM ECDSA key."
                )
            default:
                throw LancerError.keyDecodeFailed(reason: "Unsupported OpenSSH key type.")
            }
        }
        ...
        throw LancerError.keyDecodeFailed(reason: "Unsupported key format.")
    }
    ...
    public func importPrivateKey(tag: String, keyData: Data, comment: String? = nil) async throws -> PublicKeyInfo {
        guard let text = String(data: keyData, encoding: .utf8) else {
            throw LancerError.keyDecodeFailed(reason: "The selected file is not UTF-8 text.")
        }
    ...
    /// - Throws:  `LancerError.keyDecodeFailed` if parsing fails;
    ///            `LancerError.keyNotFound` / Keychain errors if the write fails.
    public func importEd25519FromPEM(
        ...
        do {
            ...
            throw LancerError.keyDecodeFailed(reason: e.errorDescription ?? String(describing: e))
        }
    ...
    public func loadEd25519(tag: String) async throws -> Curve25519.Signing.PrivateKey {
        ...
        case .ed25519(let key):
            return key
        default:
            throw LancerError.keyDecodeFailed(reason: "Stored key is not Ed25519.")
        }
    }
    ...
    public func loadPrivateKey(tag: String) async throws -> StoredPrivateKey {
        ...
        case (.ed25519, .openSSH), (.ed25519, .pem):
            throw LancerError.keyDecodeFailed(reason: "OpenSSH/PEM Ed25519 import is metadata-only in this build.")
        case (.rsa, .openSSH):
            throw LancerError.keyDecodeFailed(reason: "OpenSSH RSA import is metadata-only in this build.")
        case (.rsa, .pem):
            throw LancerError.keyDecodeFailed(reason: "PEM RSA import is metadata-only in this build.")
        case (.ecdsaP256, .pem):
            throw LancerError.keyDecodeFailed(reason: "PEM ECDSA import is metadata-only in this build.")
        case (.ecdsaP384, .pem):
            throw LancerError.keyDecodeFailed(reason: "PEM ECDSA import is metadata-only in this build.")
        case (.ecdsaP521, .pem):
            throw LancerError.keyDecodeFailed(reason: "PEM ECDSA import is metadata-only in this build.")
        default:
            throw LancerError.keyDecodeFailed(reason: "Unsupported key algorithm/format combination.")
        }
    }
    ...
    private func loadRecord(tag: String) async throws -> StoredKeyRecord {
        ...
        throw LancerError.keyDecodeFailed(reason: "Unknown key format in keychain.")
    }
