...
public actor Keychain {
    ...
    public func write(
        ...
        guard status == errSecSuccess else {
            throw LancerError.unknown(detail: "Keychain add failed: OSStatus \(status)")
        }
    }
    ...
    public func read(account: String) throws -> Data {
        if inMemory {
            guard let data = store[account] else {
                throw LancerError.keyNotFound(tag: account)
            }
            return data
        }
    ...
    public func delete(account: String) throws {
        ...
        guard status == errSecSuccess || status == errSecItemNotFound else {
            throw LancerError.unknown(detail: "Keychain delete failed: OSStatus \(status)")
        }
    }
    ...
    public func allAccounts() throws -> [String] {
        ...
        guard status == errSecSuccess else {
            throw LancerError.unknown(detail: "Keychain list failed: OSStatus \(status)")
        }
