...
public struct AddHostView: View {
    ...
    // MARK: - Source mode (BYO SSH vs Lancer Cloud)
    ...
    private enum SourceMode: Hashable, Sendable {
        ...
        case hosted   // Lancer Cloud managed runtime
    }
    ...
    private var modePicker: some View {
        let options: [(label: String, value: SourceMode)] = ProvisioningFeatureFlags.managedCloudEnabled
            ? [(label: "bring your own", value: SourceMode.byo),
               (label: "lancer cloud", value: SourceMode.hosted)]
            : [(label: "bring your own", value: SourceMode.byo)]
    ...
    // MARK: - Lancer Cloud panel (hosted)
    ...
    private var hostedPanel: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            VStack(alignment: .leading, spacing: 6) {
                ...
                Text("Run claude or codex on Lancer's managed infrastructure — no server to provision, patch, or keep online.")
                    .font(.dsSansPt(13))
                    .foregroundStyle(t.text3)
                    .lineSpacing(2)
                    .fixedSize(horizontal: false, vertical: true)
    ...
    private var hostedUpgradeCTA: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            if cloudUpgradeEligible {
                ...
                Text("Manage your Lancer Cloud subscription in Settings › Billing.")
                    .font(.dsSansPt(13))
                    .foregroundStyle(t.text3)
                    .fixedSize(horizontal: false, vertical: true)
            }
    ...
    private func generateKey() async {
        ...
        do {
            ...
            let info = try await keyStore.generateEd25519(tag: tag, comment: "lancer@iphone")
            guard !Task.isCancelled else { return }
