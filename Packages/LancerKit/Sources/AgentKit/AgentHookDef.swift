...
// and a `cmux hooks <agent>` invocation chain. Lancer only needs the schema
...
// invocation chains through `lancerd` (Lancer's remote daemon), so this
// file replaces cmux's `cmux hooks <agent>` marker with
// `lancerd hooks <agent>` and drops the cmux-only post-install actions.
...
/// which agent events to wire to which Lancer subcommands.
public struct AgentHookDef: Hashable, Sendable {
    ...
    public let sessionStoreSuffix: String            // ~/.lancer/<suffix>-hook-sessions.json
    public let disableEnvVar: String                 // e.g. LANCER_CLAUDE_HOOKS_DISABLED
    public let hookMarker: String                    // Command marker: "lancerd hooks claude"
    ...
    /// expected config shape. Lancer doesn't render the files itself — the
    /// daemon (`lancerd`) on the remote host owns that — but the iOS app
    ...
    public struct Event: Hashable, Sendable {
        ...
        public let lancerSubcommand: String // Subcommand the hook invokes (e.g. "prompt-submit")

        public init(agentEvent: String, lancerSubcommand: String) {
            self.agentEvent = agentEvent
            self.lancerSubcommand = lancerSubcommand
        }
    }
    ...
    /// home directory (Lancer reads this via `$HOME` or `pwd` over SSH).
...
extension AgentHookDef {
    ...
    public static var claude: AgentHookDef {
        AgentHookDef(
            ...
            disableEnvVar: "LANCER_CLAUDE_HOOKS_DISABLED",
            hookMarker: "lancerd hooks claude",
            format: .nested(timeoutMs: 5000),
            events: [
                .init(agentEvent: "SessionStart", lancerSubcommand: "session-start"),
                .init(agentEvent: "UserPromptSubmit", lancerSubcommand: "prompt-submit"),
                .init(agentEvent: "Stop", lancerSubcommand: "stop"),
                .init(agentEvent: "Notification", lancerSubcommand: "notification"),
                .init(agentEvent: "SessionEnd", lancerSubcommand: "session-end"),
            ],
    ...
    public static var opencode: AgentHookDef {
        AgentHookDef(
            ...
            disableEnvVar: "LANCER_OPENCODE_HOOKS_DISABLED",
            hookMarker: "lancerd hooks opencode",
            format: .nested(timeoutMs: 5000),
            events: [
                .init(agentEvent: "SessionStart", lancerSubcommand: "session-start"),
                .init(agentEvent: "UserPromptSubmit", lancerSubcommand: "prompt-submit"),
    ...
    public static var codex: AgentHookDef {
        AgentHookDef(
            ...
            disableEnvVar: "LANCER_CODEX_HOOKS_DISABLED",
            hookMarker: "lancerd hooks codex",
            format: .nested(timeoutMs: 5000),
            events: [
                .init(agentEvent: "SessionStart", lancerSubcommand: "session-start"),
                .init(agentEvent: "UserPromptSubmit", lancerSubcommand: "prompt-submit"),
    ...
    public static var cursor: AgentHookDef {
        AgentHookDef(
            ...
            disableEnvVar: "LANCER_CURSOR_HOOKS_DISABLED",
            hookMarker: "lancerd hooks cursor",
            format: .flat,
            events: [
                .init(agentEvent: "afterFileEdit", lancerSubcommand: "prompt-submit"),
                .init(agentEvent: "beforeShellExecution", lancerSubcommand: "prompt-submit"),
                .init(agentEvent: "stop", lancerSubcommand: "stop"),
            ],
    ...
    public static var grok: AgentHookDef {
        AgentHookDef(
            ...
            configFile: "lancer-session.json",
            configDirEnvOverride: "GROK_HOME",
            configDirEnvOverrideSubpath: "hooks",
            createConfigDirIfMissing: true,
            sessionStoreSuffix: "grok",
            disableEnvVar: "LANCER_GROK_HOOKS_DISABLED",
            hookMarker: "lancerd hooks grok",
            format: .nested(timeoutMs: 5000),
            events: [
                .init(agentEvent: "SessionStart", lancerSubcommand: "session-start"),
                .init(agentEvent: "UserPromptSubmit", lancerSubcommand: "prompt-submit"),
    ...
    public static var gemini: AgentHookDef {
        AgentHookDef(
            ...
            disableEnvVar: "LANCER_GEMINI_HOOKS_DISABLED",
            hookMarker: "lancerd hooks gemini",
            format: .antigravityJSON(timeoutSeconds: 5),
            events: [
                .init(agentEvent: "session.start", lancerSubcommand: "session-start"),
                .init(agentEvent: "prompt.submit", lancerSubcommand: "prompt-submit"),
                .init(agentEvent: "session.end", lancerSubcommand: "session-end"),
            ],
            aliases: ["antigravity", "agy"]
        )
    }

    /// All Lancer-supported agent hook definitions.
