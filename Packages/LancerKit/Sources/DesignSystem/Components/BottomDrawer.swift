...
public struct LancerDrawer<Content: View>: View {
    private let title: String?
    private let subtitle: String?
    private let detents: Set<PresentationDetent>
    private let surface: LancerDrawerSurface
    ...
    private var tokens: LancerTokens { surface == .workspace ? .dark : t }

    public var body: some View {
        VStack(spacing: 0) {
            LancerGrabHandle(on: surface == .workspace ? .dark : .light)
            if let title {
...
public struct BottomDrawerSheet<DrawerContent: View>: ViewModifier {
    ...
    let surface: LancerDrawerSurface
