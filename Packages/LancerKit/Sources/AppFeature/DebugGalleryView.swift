...
// UI-audit harness. Routed from AppRoot when SIMCTL_CHILD_LANCER_GALLERY is set.
struct DebugGalleryView: View {
    ...
    var body: some View {
        ...
        case "onboarding-pair":     OnboardingView(onContinue: {}, startPhase: .pair)
        case "onboarding-scan":     OnboardingView(onContinue: {}, startPhase: .scan)
        ...
        case "settings-policy":    NavigationStack { PolicyEditorView(cwd: "~/projects/lancer", initialYAML: Self.samplePolicyYAML, onReload: {}) }
    ...
    private var reviewScreen: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 0) {
                ...
                Text("Full component library → relaunch with LANCER_GALLERY=components")
                    .font(.dsMonoPt(10))
                    .foregroundStyle(t.text4)
                    .padding(16)
            }
        ...
        VStack(alignment: .leading, spacing: 0) {
            ...
            HStack(alignment: .top, spacing: 0) {
                ...
                content()
                    ...
                    .padding(12)
                    .background(LancerTokens.light.bg)
                // Dark half
                content()
    ...
    private var avatarsPanel: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            HStack(spacing: 8) {
                PixelAvatar(seed: "lancer-lock", size: 24)
                PixelAvatar(seed: "lancer-lock", size: 36)
                PixelAvatar(seed: "lancer-lock", size: 48)
                PixelAvatar(seed: "lancer-lock", size: 60)
            }
        }
    }
    ...
    private var newChatRealGallery: some View {
        NewChatTabView(
            agents: [
                DispatchAgent(id: "mac-studio|claudeCode", name: "Claude Code", cwd: "~/projects/lancer", isOffline: false, hostName: "mac-studio"),
    ...
    private var newChatGallery: some View {
        ...
        .background(LancerTokens.dark.termBg.ignoresSafeArea())
    }
    ...
    private var chatGallery: some View {
        VStack(spacing: 0) {
            ...
            ChatHeaderView(hostName: "ubuntu@dev", cwd: "~/repo/lancer", state: .streaming, onBack: {})
            ScrollView {
                VStack(spacing: 12) {
                    card(.success)
                    card(.running)
    ...
    private func mockBlock(_ d: Demo) -> Block {
        let prompt = Block.PromptInfo(cwd: "~/repo/lancer", hostName: "ubuntu@dev")
        switch d {
        case .success:
...
/// Launch with LANCER_GALLERY=orb-phases.
...
// MARK: - Block transcript review (LANCER_GALLERY=blocks)
//
...
// MARK: - Agent Island gallery (LANCER_GALLERY=hud)
...
// MARK: - Agent status header gallery (LANCER_GALLERY=statusheader)
...
private struct AgentStatusHeaderGalleryScreen: View {
    ...
    private var t: LancerTokens { .dark }
...
// MARK: - Typed Inbox gallery (LANCER_GALLERY=inbox-typed)
//
...
// MARK: - Features gallery (LANCER_GALLERY=features)
//
...
// MARK: - Keyboard gallery (LANCER_GALLERY=keyboard)
...
private struct KeyboardGalleryScreen: View {
    ...
    private let mockHistory = [
        "git status", "swift build", "cd Packages/LancerKit",
...
// MARK: - Proof Card gallery (LANCER_GALLERY=proof)
//
