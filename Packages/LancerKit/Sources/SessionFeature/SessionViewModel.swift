...
private let blockLog = OSLog(subsystem: "com.lancer.terminal", category: "BlockLifecycle")
...
public final class SessionViewModel {
    ...
    public func connect() async {
        ...
        } catch LancerError.hostKeyUnknown(let fp) {
        ...
        } catch LancerError.hostKeyMismatch(let expected, let actual) {
            try? await auditRepo?.record(
                hostID: host.id,
                type: .hostKeyChanged,
        ...
        } catch LancerError.authFailed(let reason) {
            try? await auditRepo?.record(
                hostID: host.id,
                type: .authFailure,
    ...
    /// agent reports its session ID via the lancerd protocol.
    ...
    public func attachToTmuxSession(_ name: String) {
        ...
        Task {
            let tmux = TmuxClient(session: sshSession)
            try? await tmux.attachOrCreate(name: name)
            UserDefaults.standard.set(name, forKey: "lancerLastTmuxSession_\(host.id)")
        }
    }
    ...
    private func openUnifiedShell() async {
        ...
        do {
            ...
            Task {
                ...
                if isFish {
                    UserDefaults.standard.set("fish", forKey: "lancerShellDetected")
                    ...
                    UserDefaults.standard.set("posix", forKey: "lancerShellDetected")
    ...
    public func explain(_ block: Block) -> AsyncThrowingStream<String, any Error> {
        AsyncThrowingStream { continuation in
            guard let ai = aiClient else {
                continuation.finish(throwing: LancerError.apiKeyMissing(provider: "AI"))
                return
            }
