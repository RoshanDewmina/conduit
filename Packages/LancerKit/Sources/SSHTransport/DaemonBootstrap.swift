...
public actor DaemonBootstrap {
    public static let binDir = "~/.lancer/bin"

    // Returns the path to a ready-to-run lancerd binary on the remote host.
    ...
    public static func ensureInstalled(session: SSHSession, manifest: DaemonManifest) async throws -> String {
        ...
        guard let asset = manifest.assets.first(where: { $0.os == remoteOS && $0.arch == arch }) else {
            throw LancerError.unsupportedPlatform
        }

        let binPath = "\(binDir)/lancerd-\(manifest.version)"
        ...
        guard actualSHA.lowercased() == asset.sha256.lowercased() else {
            ...
            throw LancerError.unknown(detail: "lancerd SHA-256 mismatch: got \(actualSHA), expected \(asset.sha256)")
        }
        _ = try? await session.executeCollected("chmod +x \(binPath)")
        return binPath
    }

    private static let releaseBase = "https://github.com/RoshanDewmina/lancer/releases/download/v0.1.0"
