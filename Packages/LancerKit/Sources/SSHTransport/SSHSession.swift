...
public actor SSHSession {
    public let host: LancerCore.Host
    public private(set) var isConnected: Bool = false
    public private(set) var lastError: LancerError?
    ...
    public init(host: LancerCore.Host) {
        self.host = host
    }
    ...
    private func preResolveHost() async throws {
        ...
        try await withThrowingTimeout(.seconds(2)) {
            try await withCheckedThrowingContinuation { (cont: CheckedContinuation<Void, Error>) in
                ...
                if ret != 0 {
                    cont.resume(throwing: LancerError.dnsResolutionFailed(host: h))
                } else {
                    freeaddrinfo(res)
                    cont.resume()
                }
    ...
    public func connect(credential: SSHCredential, hostKeyStore: HostKeyStore) async throws {
        ...
        } catch let e as LancerError {
            lastError = e
            throw e
        } catch {
            let mapped = LancerError.dnsResolutionFailed(host: host.hostname)
            lastError = mapped
            throw mapped
        }
    ...
    /// Runs `operation` with a deadline. Throws `LancerError.timeout` if it
    ...
    ///
    /// - Throws: `LancerError.noCredentialAvailable` when no cached credential
    ...
    public func attemptReconnect() async throws {
        ...
        guard let credential = cachedCredential,
            ...
            throw LancerError.noCredentialAvailable
        }
    ...
    public func execute(_ command: String) async throws -> AsyncThrowingStream<(Data, BlockChunk.Stream), any Error> {
        guard let client else { throw LancerError.notConnected }
    ...
    public func requestShellChannel(
        ...
        guard let writer = await writerStream.first(where: { _ in true }) else {
            task.cancel()
            throw LancerError.channelClosed
        }
        return (writer, task)
    }

    // MARK: - Exec channel (bidirectional, no PTY — used by lancerd)
    ...
    internal static func map(error: any Error, host: String) -> LancerError {
        if let error = error as? LancerError { return error }
