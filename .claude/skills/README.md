...
| **cross-platform-conversation-audit** | Multi-day Claude/Codex/Cursor conversation audits — ledger all sessions, full read per platform, independent live-repo verification pass, reconciled report. Builds on `agent-session-history-reader`; not for single-session lookups. | `references/platform-ledger-queries.md`, `references/finding-schema.md`, `references/report-template.md` |
