---
name: conduit-verification-gate
description: Use before calling Conduit work done, after Conduit code edits, when reviewing worker output, when diagnosing build/test failures, or when the user asks to verify Swift, Go daemon, iOS app-target, relay, or design-board behavior.
---

# Conduit Verification Gate
...
- ConduitKit-only Swift changes: run `swift build` and, when behavior/tests changed, `swift test` from `Packages/ConduitKit`.
...
- Daemon changes: run `go build` and `go test ./...` from `daemon/conduitd`.
