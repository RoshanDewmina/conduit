...
paths:
  - "daemon/**"
---
# Go daemon (lancerd / push-backend / lancer-mcp)

**Verify (matches CI `.github/workflows/ci.yml`, Go 1.25)** — from the module dir, not the repo root:

```bash
cd daemon/<mod> && go build ./... && go vet ./... && go test ./...   # mod ∈ lancerd, push-backend, lancer-mcp
```

**Before changing `daemon/lancerd/dispatch.go`** — the agent-CLI adapters (Claude Code, Codex,
