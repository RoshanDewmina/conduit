...
description: Create stunning, animation-rich HTML presentations from scratch or by converting PowerPoint files. Use when the user wants to build a presentation, convert a PPT/PPTX to web, create slides for a talk/pitch, turn a plan/roadmap/brainstorm into a visual deck, or explain something as a walkthrough of slides rather than a document. Helps non-designers discover their aesthetic through visual exploration rather than abstract choices.
---
...
- Motion: Use GSAP (loaded via CDN, no build step — see [animation-patterns.md](animation-patterns.md)) for staggered/sequenced reveals, timeline-driven slide transitions, and text-splitting (SplitText — free since the Webflow/GSAP merger, no license needed). Fall back to plain CSS transitions only for a single simple preview slide or when the user explicitly wants zero external scripts. Use Motion library for React when available. Focus on high-impact moments: one well-orchestrated page load with staggered reveals creates more delight than scattered micro-interactions.
...
- Also verify **surface contrast for repeated components**: any pill/chip/badge/bullet-marker that appears on multiple slide surface colors must stay visible on every one of them. Same-color-on-same-color (e.g. a green chip on a green slide) is a recurring bug class — check each variant on each surface it appears on, not just one.
...
- Single self-contained HTML file, all CSS/JS inline (GSAP loaded via CDN `<script>` tags is the one expected exception — same zero-build-step pattern as Google Fonts; see [animation-patterns.md](animation-patterns.md))
...
If the user declines, stop here. **If they choose one or both, read [sharing.md](sharing.md) and follow it** — it contains the full Vercel walkthrough, PDF export steps, and every known gotcha (non-interactive `--scope`, asset bundling, Playwright install, PDF size). Do not read sharing.md before the user opts in.

<!-- Full Phase 6 detail lives in sharing.md (lazy-loaded to keep per-invocation cost down). -->
...
| [animation-patterns.md](animation-patterns.md)     | GSAP-first animation patterns and effect-to-feeling guide            | Phase 3 (generation)      |
| [sharing.md](sharing.md)                           | Full Vercel deploy + PDF export walkthroughs and gotchas             | Phase 6, only after user opts in |
