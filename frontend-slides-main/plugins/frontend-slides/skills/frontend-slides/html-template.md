...
<head>
    ...
    <!-- GSAP (default animation engine — see animation-patterns.md). Only include the plugins actually used. -->
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        /* ===========================================
           ANIMATIONS — CSS FALLBACK ONLY (html.no-gsap)
           When GSAP loads, it is the SOLE owner of .reveal motion.
           These rules apply only if the CDN failed (offline etc.) —
           the JS adds .no-gsap to <html> in that case.
           NEVER leave un-gated CSS transitions on opacity/transform:
           they re-trigger on every GSAP tick and smear the motion.
           =========================================== */
        html.no-gsap .reveal {
            opacity: 0;
        ...
        html.no-gsap .slide.visible .reveal {
            opacity: 1;
            transform: translateY(0);
        }

        /* Fallback stagger (GSAP's stagger handles this normally) */
        html.no-gsap .reveal:nth-child(1) { transition-delay: 0.1s; }
        html.no-gsap .reveal:nth-child(2) { transition-delay: 0.2s; }
        html.no-gsap .reveal:nth-child(3) { transition-delay: 0.3s; }
        html.no-gsap .reveal:nth-child(4) { transition-delay: 0.4s; }
...
<body>
    ...
    <script>
        /* ===========================================
           GSAP GUARD — the deck must survive a failed CDN load.
           Without this guard, a 404'd or blocked script makes
           registerPlugin throw and kills the ENTIRE controller
           (navigation and stage scaling included, not just motion).
           =========================================== */
        ...
        if (hasGsap) {
            ...
            document.documentElement.classList.add('no-gsap'); // activates the CSS fallback block above
        }

        /* ===========================================
           GSAP REVEAL — see animation-patterns.md for the full pattern set
           (stagger objects, SplitText headlines, Flip continuity, matchMedia)
           =========================================== */
        function revealSlide(slide) {
            if (!hasGsap) return; // CSS fallback animates via .visible instead
            const targets = slide.querySelectorAll('.reveal');
            gsap.killTweensOf(targets); // rapid navigation otherwise strands elements mid-tween
            const tl = gsap.timeline({ defaults: { duration: 0.65, ease: "power3.out" } });
            tl.from(targets, { autoAlpha: 0, y: 24, stagger: 0.12 });
            return tl;
        }
        ...
        class SlidePresentation {
            ...
            showSlide(index) {
                ...
                this.slides.forEach((slide, i) => {
                    // .active/.visible still control visibility/pointer-events per viewport-base.css —
                    // GSAP only drives the .reveal children's motion, never slide visibility itself.
                    slide.classList.toggle('active', i === this.currentSlide);
                    slide.classList.toggle('visible', i === this.currentSlide);
                });
                revealSlide(this.slides[this.currentSlide]);
            }
        }
...
1. **SlidePresentation Class** — Main controller with:
   ...
   - `showSlide()` plays a GSAP reveal timeline on the incoming slide (see `revealSlide()` above and animation-patterns.md) — CSS `.active`/`.visible` classes still own visibility/pointer-events per viewport-base.css; GSAP only animates the `.reveal` children
   - The GSAP guard (`hasGsap` + `html.no-gsap` fallback class) is mandatory — a deck must degrade to CSS reveals, not die, when the CDN is unreachable
