...
## Context
Repo `/Users/roshansilva/Documents/command-center`. Backend: `daemon/push-backend/` (Go). iOS build: `cd Packages/ConduitKit && swift build`. Read `docs/SERVER.md`.

**Confirmed problems:**
- Push backend URL is **hardcoded** to `http://35.201.3.231:8080` in `Conduit/ConduitApp.swift:14` — **plain HTTP** (ATS rejects this in Release) on a raw IP. Cloud Run gives auto-HTTPS + a stable URL + scale-to-zero.
...
2. **Secret Manager** — create `APPROVAL_RELAY_SECRET` (strong random value; backend refuses production startup without it), `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, `STRIPE_PRICE_MONTHLY`, `STRIPE_PRICE_ANNUAL`, `APNS_KEY` (the `.p8` file), `APNS_KEY_ID`, `APNS_TEAM_ID`, `PUBLIC_BASE_URL`, `WEBSITE_BASE_URL`. Wire into the service and **mount the `.p8` as a volume file** at `/secrets/apns.p8` matching `APNS_KEY_PATH`:
   ...
   ```
3. **iOS: kill the hardcoded IP.** Move the URL to build config — add `CONDUIT_PUSH_BACKEND_URL` to `project.yml` `settings.base` and `Info.plist`, then in `ConduitApp.swift:14` read `Bundle.main.infoDictionary?["CONDUIT_PUSH_BACKEND_URL"] as? String ?? ""`. No plain-HTTP fallback in Release.
4. **Improve the remote-notification handler** — return `.newData` and post a `NotificationCenter` broadcast so the Inbox updates in background; add `Notification.Name.conduitRemoteApprovalReceived` to `NotificationsKit/Notifications.swift`.
...
- Keep `fly.toml` intact (no Fly migration). Don't break the existing conduitd path.
...
### iOS URL: <project.yml + Info.plist + ConduitApp.swift:14 change; no plain-HTTP in Release?>
