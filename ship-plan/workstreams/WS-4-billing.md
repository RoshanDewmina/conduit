...
## Context
Conduit ships a **paid v1** (locked decision): a StoreKit 2 one-time purchase **plus** a US-only external Stripe subscription path. Repo `/Users/roshansilva/Documents/command-center`. iOS build: `cd Packages/ConduitKit && swift build`. Go backend: `daemon/push-backend/` (`go build`).

**Confirmed state:**
- StoreKit 2 one-time `dev.conduit.mobile.pro` ($14.99) fully wired: `PurchaseManager.swift` (products ~L57–71, `purchase()` ~L73–96, `Transaction.updates` ~L121–130). 6 features gated behind `isPro` (`SessionShellView.swift` L140–166; multi-host in `AppRoot.swift onAddHostGated`). `PaywallSheet.swift` complete.
...
- **`isPro` DEBUG bypass:** `PurchaseManager.swift:34–41` returns `true` unless `conduitDebugProBypass=false` (Settings→Terminal→Debug). **Must not be true in Release.**
...
1. **Finish `billing.go`** — complete all four routes:
   ...
   - `/billing/return` — redirect to the `conduit://billing/return?session_id=…` deep link.
...
4. **Deep-link handling** — confirm the app handles `conduit://billing/return?session_id=…` and refreshes entitlement on return.

## Owner-only (do in the Stripe dashboard — list these in your report, don't fake them)
Create "Conduit Pro" product + monthly/annual prices (copy IDs → env); create the webhook endpoint (URL = the Cloud Run URL from WS-5 + `/billing/webhook`; copy signing secret → `STRIPE_WEBHOOK_SECRET`).
...
### Deep link conduit://billing/return: <handled?>
