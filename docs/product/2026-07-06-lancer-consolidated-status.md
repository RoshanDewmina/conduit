...
- Tab bar / Control / Activity roots are vestigial. **Cursor shell** (`AppFeature/CursorStyle/`, `LANCER_CURSOR_SHELL` / `LANCER_CURSOR_SHELL_LIVE`) is the current navigation target; legacy sidebar / Command Home is deprecated.
...
| **Navigation shell** | **Cursor shell** (`CursorAppShell` under `AppFeature/CursorStyle/`). `LANCER_CURSOR_SHELL=1` (mock), `LANCER_CURSOR_SHELL_LIVE=1` (live bridge). Legacy sidebar / Command Home deprecated. |
...
| **Relay E2E harness** | **PASS** (2026-07-06) — `relay-approval-e2e.sh` through live Cursor shell; see [proof run](../test-runs/2026-07-06-tier-0-live-cursor-shell-proof.md). |
