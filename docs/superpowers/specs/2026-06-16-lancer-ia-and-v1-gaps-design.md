# Lancer — IA + v1 Gap-Closure Design Spec
...
Lancer has ~120 catalogued surfaces in the design board but no settled v1 information architecture, and several capabilities that competitors ship are missing or unscoped. This spec locks the navigation model, folds redundant surfaces, picks the v1 gap features worth building now, and sets the resilience scope — so Swift implementation can start against a stable target.

**North star:** Lancer is *the governed way to let agents act on your machines* — agent runs on the host, phone is the remote control with a policy engine, tamper-evident audit, and Face-ID risk gating. We close parity gaps without diluting that governance moat.
...
**v1 commitment (A+B):**
- **A — Host-side detached persistence:** the agent survives the phone disconnecting (it already runs under lancerd on the host; harden this so a dropped relay/SSH link never kills the run).
...
**The dealbreaker question:** if a user starts a bare `claude` (or any agent) in their own terminal — *not* launched from Lancer — does that session appear in Lancer and can they continue it from the phone?

**Today: no.** lancerd only controls agents *it* spawned — it owns their PTY, parses their OSC-133 block markers, and installs the approval-firewall interception at launch. A `claude` started in Terminal.app/iTerm is a child of that terminal's shell with a PTY lancerd has no handle to. No PTY handle ⇒ no input injection, no approvals, no blocks.
...
**Three mechanisms for Lancer (recommendation = A primary + B visibility):**

- **A — Launch-through-Lancer shim (RECOMMENDED, primary).** The host installer drops a frictionless `claude` shell alias / `lancer run` wrapper that routes the launch through lancerd. lancerd owns the PTY from byte zero ⇒ full control, approvals, blocks, and §4 resilience for *every* session the user starts. Cost to user ≈ zero (alias is invisible); cost to us = installer + wrapper plumbing we largely already have.
- **B — Read-only session discovery (RECOMMENDED, visibility layer).** lancerd watches the agent's on-disk transcript store (e.g. Claude Code's `~/.claude/projects/**/*.jsonl`) and surfaces *externally-started* sessions in Fleet as read-only mirrors — you can *watch* a bare session from the phone even if you can't drive it. "Take over" then prompts to attach (via C) or relaunch through the shim.
- **C — tmux/multiplexer adoption (DEFER).** If the bare session runs inside tmux, lancerd can `pipe-pane` to mirror output and `send-keys` to inject input → genuine read/write takeover of an externally-started process. Full control of bare non-tmux sessions is otherwise not possible without ptrace-grade hacks. Defer unless users demand driving non-shim, non-tmux sessions.

This is a sibling to §4 — call the bundle **Session Continuity**: §4 keeps a *Lancer-owned* session alive across drops; §6 brings *externally-started* sessions into Lancer.
...
- **A — Launch-through-Lancer shim (PRIMARY, v1).** Build it as **three layers** — PATH-level shim binary + shell function + managed env var (`LANCER_CLAUDE_WRAPPER_SHIM`) — copying cmux's proven approach. This is the only way to survive `env claude`, non-interactive shells, user aliases, and IDE-spawned agents. The shim is also the *single biggest risk* (silent failure across shells) → ship a `lancer doctor` wrapper-coverage check.
...
- **Takeover via `--resume`, NOT tmux injection (v1 — replaces old "C-as-takeover").** "Take Over" reads the bare session's transcript, then **relaunches it under lancerd with `claude --resume <session-id>`** (universal across all three agents). The continued session runs with the **approval firewall + audit chain fully intact** — this *resolves* the governance-collision caveat instead of accepting it. Limitation: resumes at a conversation boundary, not mid-turn; if the bare agent is mid-execution the user waits or kills it.
- **tmux = persistence container, not adoption driver (v1).** Spawn lancerd-owned agents inside a named tmux session (`lancer-<id>`) so the run survives lancerd restart and is re-attachable. This is §4's process-survival mechanism.
...
- **Transport resilience (Mosh/QUIC) deferred to v2.** Tailscale/WireGuard already roams at the network layer; v1 = WireGuard tunnel + lancerd stream buffering. (Refines §4-B: roaming *transport* is mostly already handled; v1 effort goes to process persistence + buffering, not a custom transport.)
...
**Host-side notes:** `node_modules`/`target`/`.build` are duplicated per worktree (the #1 disk trap) → lancerd should lean on pnpm/cargo shared stores. lancerd uses Claude Code's native worktree support when present, else manual `git worktree add`, falling back per-agent for Codex/Gemini — invisible to the phone UI.
