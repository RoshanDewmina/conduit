# Conduit Live-Loop E2E Test — Design (Pass 1: local governed-approvals loop)

**Date:** 2026-06-12
**Author:** Claude (with owner)
**Status:** Approved design → execution
**Scope:** Prove the *real* governed-approvals loop end-to-end on this Mac with a **real Claude Code agent**, the **conduitd** daemon, the **policy engine**, and the **iOS app** (simulator). Real APNs push (app-closed) is explicitly **out of scope** for Pass 1 — that is Pass 2 on a physical device.
...
```
 iOS app (sim)  ──SSH exec──▶  conduitd serve  ──unix socket──▶  conduit-hook.sh
...
- `conduitd serve` is one process that listens on `~/.conduit/conduitd.sock` (hooks) **and** speaks framed JSON-RPC over stdin/stdout (phone). — `daemon/conduitd/server.go:262,294,302,324-353`
- The iOS app launches it verbatim: `bash -c '$HOME/.conduit/bin/conduitd serve'` over an SSH exec channel, then a `conduit.device.register` handshake mints a per-session relay token. — `Packages/ConduitKit/Sources/SSHTransport/DaemonChannel.swift:28-50,107`
- The Claude Code PreToolUse hook (`docs/conduit-hook.sh`) auto-allows read-only tools, maps Bash→high/command, Write/Edit→medium/fileWrite, then calls `conduitd agent-hook approval …`. Exit 0 = proceed, exit 2 = stop.
- Policy engine (`daemon/conduitd/policy/`) evaluates allow/deny/ask from `~/.conduit/policy.yaml` (`policy/load.go:11`).

Tools confirmed present on this Mac: `claude` 2.1.176 (`/opt/homebrew/bin/claude`); `conduitd-darwin-arm64` runs (`version → 0.1.0`); simulator iPhone 17 Pro booted; Remote Login on; localhost SSH password in Keychain (`conduit-localhost-ssh`).

## 3. Topology & isolation decision

**Topology:** ① App-launched `conduitd serve` over SSH (exactly what `DaemonChannel.start()` does). The resident `daemon` (launchd, survives-disconnect) mode is **deferred** to Pass 1.5/2.

**Method:** ③ staged isolation — prove each leg before the full loop, so failures localize.

**Isolation (the key safety constraint):** the test harness *is* a running Claude Code session. A **global** conduit hook (`~/.claude/settings.json`) would route this session's own Bash/Edit calls through conduitd and could wedge it the moment `serve` starts. Therefore:
- Install the real artifacts at their real paths: binary `~/.conduit/bin/conduitd`, hook script `~/.claude/hooks/conduit-hook.sh`, policy `~/.conduit/policy.yaml`.
...
- Both the app-launched `serve` and the test `claude` run as the same user → same `$HOME` → same socket `~/.conduit/conduitd.sock`. Leg alignment holds.
...
A real `~/.conduit/policy.yaml` (mirrors the `cc-policy` preset) chosen so a single agent run exercises all three behaviors:
...
1. **Pre-flight:** confirm `claude`, conduitd binary, Remote Login, booted sim, Keychain password. Snapshot the current global `~/.claude/settings.json` (so we can prove it was untouched).
2. **Install (real paths):** copy conduitd → `~/.conduit/bin/conduitd`; copy hook → `~/.claude/hooks/conduit-hook.sh` (`chmod 700`); write `~/.conduit/policy.yaml` fixture.
3. **Test workspace:** create `/tmp/conduit-e2e-workspace/` (a throwaway git repo) with `.claude/settings.json` wiring the PreToolUse hook **scoped to this project only**.
4. **Leg (a) — plumbing smoke (③):** with `serve` running, fire a synthetic `conduitd agent-hook approval --kind fileWrite --command "…"` and confirm the event delivers and a decision returns. (De-risks the socket/serve path with no agent.)
5. **Leg (b) — policy unit:** drive `allow`/`deny`/`ask` events through conduitd directly and confirm correct effects + audit entries (Go tests already cover this; spot-check live).
...
9. **Capture & report:** stop recording; collect screenshots + the conduitd audit log; write the test-run report.
...
- ✓ App launches `conduitd serve` over SSH and completes `conduit.device.register`.
...
- **Hook needs `python3`** (the parser in `conduit-hook.sh`) → confirm `python3` present in pre-flight.
...
- `docs/test-runs/2026-06-12-live-loop-pass1.md` — report with the conduitd audit log pasted as evidence and an honest pass/fail per §6.
