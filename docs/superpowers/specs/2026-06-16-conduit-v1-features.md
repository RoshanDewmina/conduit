...
Conduit is **the governed way to let agents act on your machines**. The agent runs on the user's own host via the resident daemon `conduitd`; the phone is the remote control. The moat is governance: policy engine, tamper-evident audit chain, blast-radius risk scoring, and Face-ID/WebAuthn gating.
...
### Session Continuity
- A — Launch-through-Conduit shim (PATH binary + shell function + env var)
- B — Read-only transcript discovery of bare sessions
- Takeover via `--resume` (relaunch under conduitd, governance-preserving)
- tmux as persistence container for conduitd-owned agents
...
| Board Page | v1 Role |
|---|---|
| Conduit v1 Features.dc.html | ✅ Spec overview |
| Conduit Onboarding.dc.html | ✅ v1 |
| Conduit Pairing Hosts SSH.dc.html | ✅ v1 |
| Conduit Fleet.dc.html | ✅ v1 (worktree/agent cards + quota rings) |
| Conduit Quota Rings.dc.html | ✅ Gauge component reference |
| Conduit Agent Worktree Detail.dc.html | ✅ Drill-down from Fleet |
| Conduit New Chat.dc.html | ✅ v1 |
| Conduit Session Chat.dc.html | ✅ v1 (blocks + voice) |
| Conduit Voice Input.dc.html | ✅ Two-way voice surface |
| Conduit Inbox.dc.html | ✅ Pending approvals only |
| Conduit Diff Review.dc.html | ✅ Mobile unified diff |
| Conduit Activity Audit.dc.html | ✅ 4th tab governance timeline |
| Conduit Live Activity.dc.html | ✅ Hybrid A+B design captured |
| Conduit Settings.dc.html | ✅ v1 |

### Beyond-v1 board pages
| Board Page | New Home |
|---|---|
| Conduit Approvals.dc.html | 🔄 Absorbed into Inbox + Diff Review |
| Conduit Activity.dc.html | 🔄 Replaced by Activity/Audit tab |
| Conduit Git Files Preview.dc.html | ⏸ Deferred |
| Conduit Agent Cloud Hosted.dc.html | ⏸ Beyond v1 |
| Conduit Worktrees.dc.html | ⏸ Beyond v1 (full parallel fleet; single-agent worktree in Fleet v1) |
| Conduit Quota Spend.dc.html | 🔄 Partial — quota rings in Fleet/Settings v1; full spend analytics beyond |
| Conduit Platform Surfaces.dc.html | 🔄 Partial — rings now; widgets/post-v1 |
| Conduit Design System.dc.html | 📚 Reference (not a product surface) |
