# Lancer "Bridge" Platform — Future Roadmap

> **Type:** strategic roadmap / vision (not a TDD implementation plan). The near-term UI migration has its own
> doc (`docs/audit/LANCER_OVERVIEW_BOARD_MIGRATION_PLAN.md`); the iOS implementation plan is written after the
> design board is re-approved. This roadmap captures *where the product is going* and *what unlocks it*.

**Date:** 2026-06-13
**Source:** synthesis of this conversation + `docs/LANCER_PROJECT_DOSSIER.md` + codebase grounding.

---

## 1. Vision

`lancerd` — "the bridge" — is the product, not a phone accessory. It sits in a rare position: at the agent's
...
| **Lancer (target)** | **no** | **no (can be local)** | **no (E2E)** | **any (adapter SPI)** |
...
1. **Default — Lancer-hosted E2E relay:** zero config, any network. (Mass adoption.)
2. **Self-hosted relay:** enterprise runs the relay container; nothing touches Lancer infra. (Regulated/air-gapped; OSS trust.)
...
capability below. Foundation already exists in part: `lancerd` POSTs approvals outbound to `push-backend → APNs`,
...
**Integration point for every agent is the same:** its permission / tool-approval callback. Lancer already has a
...
vendor. A new agent = one small adapter. Open-sourcing `lancerd` makes the community the integration team (the
...
| Tier | Theme | Notes |
|---|---|---|
| **0 · Foundations** | (a) **E2E bidirectional relay** · (b) **account registry + per-vendor quota/auth readers** · (c) **daemon control protocol + `lancer` user CLI** · (d) **Adapter SPI** | Unlocks everything + onboarding + OSS positioning. The CLI is "a program on its own." |
...
account registry). Implementation plan: `docs/superpowers/plans/2026-06-13-lancer-run-control.md`.
...
- **Class A — external pre-tool hook (lancerd already owns this seam):** Claude Code, Codex,
  opencode, **and Gemini**. Adding a Class-A vendor = copy the hook script + a `hooks.json` fragment +
  a `normalizeAgentSource` ID + an optional status reader. lancerd's **`agent-hook` CLI *is* the SPI**
...
- **Class B — closed/internal approval, no attach point:** goose, aider, Cline, RooCode, Kilo. The
  **only** bridge for the whole set is a single **`lancer-mcp` gateway** that wraps dangerous tools as
