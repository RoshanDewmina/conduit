...
- **A3.** ~~Fix the daemon `status=failed exitCode=1, zero output` bug~~ **DONE 2026-07-19**
  (PR #179 — drain-before-failed-status + bounded-tail fallback; root cause `cmd.Wait()` closed
  pipes before stderr drain).
...
- **B2.** ~~Emergency Stop atomic daemon-side primitive~~ **DONE 2026-07-19** (PR #178 —
  persisted `emergency_stop.json` latch; denies new/late gates ahead of policy eval; explicit
  `agent.emergencyStop.clear` sole un-latch; OS-level process-group kill). Device re-proof still
  owner-gated under B1.
...
**Monetization launch-prep only (decision 6 — no new billing products):**
- Founder's Edition IAP framing + public pricing page (free / Founder's / subscription later).
- Rebrand local `Lancer.storekit` display strings → "Founder's Edition" at **$79–99** (ASC
  price pick in band); create ASC record for existing `dev.lancer.mobile.pro` at chosen tier.
...
- Public pricing page (new `marketing/` or static site — not the `web/` fleet app).
