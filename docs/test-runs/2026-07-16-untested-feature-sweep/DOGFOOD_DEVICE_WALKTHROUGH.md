...
| **WT-H** | **FIXED same night** ([PR #154](https://github.com/RoshanDewmina/conduit/pull/154), `fix/observed-thread-live-follow`). Was: observed threads fetched once on open AND daemon `attachObservedSession` was snapshot-once (re-attach imported nothing — thread frozen at first attach forever). Fix: daemon delta-import on re-attach + iOS 1.5s observed-follow poll (Orca-subscription analogue). **Owner live-proved on phone** ~20:37 ET: desktop marker message appeared in the open thread without re-entering | ~~P2~~ FIXED | Owner live repro 7:52 PM; live-PASS 20:37 ET; go -race + 12/12 bridge tests (sim lease-199) |
| **WT-I** | **Hook/system artifacts leak into the phone transcript** — raw `stop_hook_summary{"type":"pr-link",…}` JSON rendered inline as message text in an observed desktop thread | P2 | Owner screenshot 7:52 PM (in session chat); needs a transcript-decode filter for hook-summary lines |
| **WT-J** | **Threads open scrolled to the TOP, not the latest message.** Mechanism found: initial transcript load fires `scrollToTailIfFollowing` (`LiveThreadView.swift:164`), which is gated on `isNearBottom` — false at open on a long thread, so the scroll never happens; owner must tap the scroll-to-bottom arrow every time. Fix: unconditional `scrollToTail` on first transcript population (keep the `isNearBottom` gate for subsequent updates) | P2 | Owner report 7:55 PM ("always have to click the arrow"); code mechanism at `LiveThreadView.swift:162-185`, `549-551` |
