...
| **WT-H** | **Observed desktop threads never auto-refresh.** Transcript is fetched once on open (`attachObservedSession` full-history path); the ~1s `LivePollPolicy` poll runs only while a phone-dispatched run is in flight. New desktop-side messages appear only after backing out and re-opening the thread | P2 | Owner live repro 7:52 PM (open Desktop thread stayed on a stale message after new desktop activity); `LivePollPolicy.swift:8`, `ShellLiveBridge.swift` attach path |
