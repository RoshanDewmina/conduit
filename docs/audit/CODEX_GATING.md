# Codex Gating Analysis — Conduit PreToolUse Hook Coverage

## Executive Summary

**Risk: HIGH.** Currently, Conduit's PreToolUse gating hook covers **only Claude Code**.
Dispatched codex agent runs have NO approval gating through Conduit. Adding the
...
- **Codex:** There is NO installer for a codex PreToolUse hook in `hook_install.go`.
  ...
  script to `~/.codex/hooks/conduit-hook.sh` but:
  ...
  3. The codex hook script itself (`docs/codex-conduit-hook.sh`) would need to exist.
...
- **With Claude Code:** Conduit's PreToolUse hook intercepts every tool call (Bash,
...
- **With codex (currently):** There IS no Conduit PreToolUse hook for codex. Even if
...
- **Explicit opt-in:** user sets `CONDUIT_CODEX_UNSAFE=1` → codex runs ungated.
...
1. **Codex PreToolUse hook parity:** Either codex supports a similar PreToolUse hook
   mechanism, or Conduit must intercept codex tool calls at a different layer (e.g.,
   wrapping the `codex` binary with a shim that implements the gating).

2. **installer coverage:** `conduitd install` must wire the codex hook identically
   to how it wires the Claude hook — script + settings merge.

3. **Verification:** `conduitd doctor` must check both Claude AND codex hook wiring.