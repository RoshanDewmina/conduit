# Lancer — "Overview Board" UI Migration Plan

**Date:** 2026-06-13
**Target design:** `Lancer Overview Board.html` (+ `cc-*.jsx`, `lancer.css`) in the design handoff
...
verify + install) **exists but is never called.** Today `DaemonChannel.start()` assumes `lancerd` is
**already present** at `$HOME/.lancer/bin/lancerd`. So the bridge is neither auto-installed over SSH
...
- **Now:** keep SSH-paste as the working path; simplify the current **7-step** onboarding toward the
  ...
  installs the bridge (lancerd) that enforces your policy and survives disconnects."
- **Infra track (separate, gated, sequenced):** (1) **wire `DaemonBootstrap.ensureInstalled()` into the
  connect flow** so connecting actually installs/repairs `lancerd` over SSH (today it's assumed
...
> LancerKit + full app target build green. Remaining open items called out inline below.
...
**Build loop:** `cd Packages/LancerKit && swift build` after each change; XcodeBuildMCP
...
- Never imply Lancer Cloud is required for BYO-host approvals.
...
- **Bridge install is unwired** (§6 corrected): `DaemonBootstrap.ensureInstalled()` exists but is never
  called; `lancerd` is assumed pre-installed. Wiring it is the first infra-track task.
