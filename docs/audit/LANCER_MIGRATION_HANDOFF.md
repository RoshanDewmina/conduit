# Lancer — UI Migration Handoff (Overview Board)
...
> 2. `docs/audit/LANCER_OVERVIEW_BOARD_MIGRATION_PLAN.md` — the approved plan; §8 has live phase status.
> 3. `docs/audit/LANCER_UI_CONSISTENCY_RULES.md` — **binding** design rules (R1–R8). Every screen obeys these.
...
- **The migration is ~Phases 0–3 done and in-sim verified.** LancerKit (`swift build`) **and** the full
...
## 1. What Lancer is (orientation)

Lancer is an **iOS SSH / AI-agent management app**. The product thesis: coding agents (Claude Code,
Codex, opencode, …) running on your machines **pause for risky actions**; Lancer relays the approval
...
- **UI:** 100% SwiftUI, **Swift 6** (strict concurrency on). No web renderer — the only way to see UI is
  the **iOS Simulator**.
- **Package:** `Packages/LancerKit` (SPM, ~100 files) holds all UI + most logic, split into modules
  (`AppFeature`, `InboxFeature`, `SettingsFeature`, `OnboardingFeature`, `SessionFeature`,
  `WorkspacesFeature`, `KeysFeature`, `DesignSystem`, `LancerCore`, `AgentKit`, `SSHTransport`,
  `SecurityKit`, `PersistenceKit`, `NotificationsKit`, `TerminalEngine`, …). The Xcode app target
  (`Lancer.xcodeproj`, scheme `Lancer`) wraps it.
- **Backend bridge:** a Go daemon `lancerd` (`daemon/`) installed on the user's host enforces policy
...
Full text: `docs/audit/LANCER_UI_CONSISTENCY_RULES.md`. The ones that drive most decisions:
...
**Design tokens** live in `Packages/LancerKit/Sources/DesignSystem/Tokens.swift`:
- `@Environment(\.lancerTokens) private var t` in every view.
...
- `LancerTokens.spectrumColors` = the red→blue strip; `SpectrumBar` is the animated decorative version.
...
1. **LancerKit-only inner loop (fast, ~2–6s):**
   ```bash
   cd /Users/roshansilva/Documents/command-center/Packages/LancerKit && swift build
   ```
   Use after every edit. **Caveat:** SPM does NOT catch all strict-concurrency / module-visibility
   breaks. Example this session: `OnboardingView` referenced `AutonomyPreset` (in `LancerCore`) without
...
```bash
cd Packages/LancerKit && touch Sources/<EditedModule>/<File>.swift && swift build
```
...
6. **Phase B (infra-gated, do NOT block UI on this):** wire `DaemonBootstrap.ensureInstalled()` into the
   connect flow (today `lancerd` is **assumed pre-installed** — real infra gap), publish
...
session default, bundle id `dev.lancer.mobile`). Confirm with `session_show_defaults`.
...
**Three launch modes:**
1. **Real app, jump to a tab:** `env: { "LANCER_TAB": "inbox|fleet|activity|settings" }`. This is the
...
2. **Gallery harness (mock UI, no SSH):** `env: { "LANCER_GALLERY": "<route>" }`. Routes (see
   ...
   NO `LANCER_GALLERY` key at all.
3. **Live SSH block session** (`LANCER_GALLERY=session` + `LANCER_TEST_*`): see `CLAUDE.md` →
   "Running the live block session." Needs macOS Remote Login (sshd) on + the login password in Keychain
   (`security find-generic-password -s lancer-localhost-ssh -w`). Harnesses auto-trust the first host
...
- **Module visibility footgun:** types in `LancerCore` (e.g. `AutonomyPreset`, `ApprovalBlastRadius`,
  `AgentVendorStatus`) need an **explicit `import LancerCore`** even when transitively linked — SPM may
...
- **`launch_app_sim` needs `bundleId` in session defaults** — it's set (`dev.lancer.mobile`); if you
  ever see "bundleId is required," call `session_set_defaults({bundleId:"dev.lancer.mobile"})`.
- **Device builds:** code signing + DeviceTesting entitlements apply; iCloud gated by
  `LANCER_ICLOUD_ENABLED`; sim-only bugs hide on device. Prefer XcodeBuildMCP device tools over raw
  `xcodebuild`.
...
5. Each agent **reports raw facts** (files touched, new public signatures, anything it couldn't source)
   — the orchestrator integrates and fixes the seams (e.g. the missing `import LancerCore`, the
...
- Tokens / rules: `DesignSystem/Tokens.swift`; `docs/audit/LANCER_UI_CONSISTENCY_RULES.md`.
...
- Backend bridge: `daemon/` (Go `lancerd`); `SSHTransport/DaemonChannel.swift`
...
2. `cd Packages/LancerKit && swift build` then `mcp__XcodeBuildMCP__session_show_defaults` +
...
5. Screenshot each change light+dark via the `LANCER_TAB` real-app harness (§7) before committing.
