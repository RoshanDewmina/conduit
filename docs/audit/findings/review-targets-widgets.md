...
**Scope:** Conduit app target, ConduitWidget, ConduitLiveActivityWidget, ConduitWatch, ConduitWatchWidget, `project.yml`, plus the glue in ConduitKit that those targets exercise for **Governed Approvals v1** (push registration, notification actions, Live Activity, WatchConnectivity approval transfer).
...
| 7 | **Release-hygiene compile-out** | ✅ **PASS** | `isPro` bypass, `DebugSeeder`, debug host auto-trust are all `#if DEBUG` (verified file-level guard + call-site guard). No shipping "REVIEW" pill found (only the DEBUG `CONDUIT_GALLERY=review` route). Production TOFU host-key prompt intact (`autoTrustHostKey` defaults `false`). |
...
**[BLOCKER] Conduit/Info.plist (+ project.yml:42-66) — `NSMicrophoneUsageDescription` and `NSSpeechRecognitionUsageDescription` missing while the dictation feature requests microphone + speech recognition.**
- `Packages/ConduitKit/Sources/SessionFeature/DictationEngine.swift:19-36` calls `SFSpeechRecognizer.requestAuthorization` (line 21) and activates `AVAudioSession(.record)` + an `AVAudioEngine` input tap (lines 28, 36). iOS **terminates the process** if the corresponding purpose string is absent when the permission is requested.
...
- **Proposed fix:** add to the Conduit target `info.properties` in `project.yml`:
  - `NSMicrophoneUsageDescription` — e.g. "Conduit uses the microphone to dictate terminal commands by voice."
  - `NSSpeechRecognitionUsageDescription` — e.g. "Conduit transcribes your speech on-device/Apple servers to let you dictate commands." Then regenerate the project.
...
- The governed-approvals headline surface uses App Intents: `ApprovalActionIntent` (`SessionFeature/ApprovalActionIntent.swift:20`, a `LiveActivityIntent`) for the Approve/Reject buttons in `ConduitLiveActivityWidget/ConduitLiveActivityWidget.swift:59-79,114-133`, and `ConduitWidgetIntent` (`AppIntentConfiguration`) in `ConduitWidget/ConduitStatusWidget.swift:13,51`. Disabling metadata extraction skips generating the `Metadata.appintents` bundle the system uses to discover/resolve intents across the process boundary.
...
- **Proposed fix:** remove the project-base override (default is `YES`), or set `ENABLE_APP_INTENTS_METADATA_EXTRACTION = YES` on the Conduit app + `ConduitWidget` + `ConduitLiveActivityWidget` targets. Verify the buttons fire `perform()` in a Release build on device.
...
- The buttons in the Live Activity render only `if let approvalID = context.state.pendingApprovalID, !approvalID.isEmpty` (`ConduitLiveActivityWidget.swift:57,112`). `updatePendingApprovals` preserves `status/agentName/isStreaming` from `lastContent` but **omits `pendingApprovalID` → nil**.
...
- **Reachability: HIGH** for any watch decision while a session is connected (decision reaches conduitd, but local state diverges).
...
**[MAJOR] ConduitWatch/WatchConnector.swift:41-46 + WatchStore.swift:57-62 — watch decision send is best-effort `sendMessage` only (no `errorHandler`, no guaranteed-delivery fallback) while the UI optimistically clears the row → silent loss of a watch "Allow".**
...
- Adversarial pass: it **fails safe security-wise** (conduitd's 120 s timeout auto-**denies**, never auto-approves). But a user's **Approve** is silently lost when the phone is unreachable — the headline "approve from your wrist" path is unreliable, and the watch claims success.
...
**[m1] SessionFeature/ApprovalRelay.swift:67-75 — exactly-once is not guaranteed client-side; the relay does at-least-once and relies on conduitd idempotency.**
...
- Adversarial pass: safe **iff** conduitd dedups by `approvalId` (and the design's 120 s auto-deny is the fail-safe backstop). Duplicate audit rows are also produced.
- **Proposed fix:** confirm/Document conduitd idempotency on `approvalId`; consider a client-side "already-responded" guard keyed by `approvalId` to avoid duplicate backend+SSH sends and duplicate audit entries.

**[m2] project.yml:90,131,167,193,227 vs Conduit/Info.plist:17-18 (and every extension Info.plist) — `MARKETING_VERSION 0.1.0` never reaches the bundle; `CFBundleShortVersionString` is hard-coded `1.0`.**
...
**[m3] Conduit/PrivacyInfo.xcprivacy:51-62 — CrashData collection is declared, but Sentry never starts (empty DSN), so no crash data is actually collected/sent.**
- `ConduitApp.swift:33,44-53`: `sentryDSN = ""` → `configureSentry()` returns early; SDK never initializes. Over-declaration isn't a rejection risk, but the manifest is inaccurate.
...
**[m4] Conduit/PrivacyInfo.xcprivacy:7-15 — FileTimestamp reason `C617.1` ("display timestamps to the person") may not match the actual call site.**
...
**[m5] Conduit/Info.plist:38-42 — `NSAllowsLocalNetworking` is set and Conduit is an SSH client to user-entered hosts (often LAN), but there is no `NSLocalNetworkUsageDescription`.**
...
- **Proposed fix:** if LAN hosts are supported, add `NSLocalNetworkUsageDescription` (e.g. "Conduit connects to SSH servers on your local network."). Verify on a physical device against a LAN host.

---

## NIT

**[nit1] ConduitWatch/Assets.xcassets/AppIcon.appiconset/Contents.json — watch icon declares only a single `watch-marketing` 1024×1024.** Modern Xcode supports single-size icons; verify the watch app icon renders/validates for the watchOS 26 deployment target at archive time.

**[nit2] Conduit.entitlements:5-6 / project.yml:77 — `aps-environment: production` is used for both Debug and Release (no per-config override).** Correct for App Store/TestFlight; just note local dev-device push needs a `development` token (works fine for submission).
...
- **Widget data contract**: phone writes `WidgetSnapshot` keys (`SessionViewModel.swift:542-548`) that `ConduitWidget/ConduitStatusWidget.swift:34-44` reads (same app group + keys). Watch `InboxCountWidget.swift:28-31` reads `watchPendingCount`, written by `WatchStore.swift:78-80` on-watch — consistent within the watchOS sandbox. Timeline refresh policy (15 min + `reloadAllTimelines()`) is reasonable.
