...
**Scope:** Go only — `daemon/push-backend/**` and `daemon/conduitd/**` (two separate modules). No Swift, no `project.yml`. Branch `feat/governed-approvals`, worktree `governed-approvals-audit`.
...
**Status:** B2 complete. conduitd poll-path audit/policy gap closed. `go vet` / `go test` / `go test -race` green in BOTH modules.
...
|---|---|---|---|---|
| **1 — control plane** | `APPROVAL_RELAY_SECRET` (deployment-wide) | `POST /register`, `POST /approval`, `POST /run-complete` | `relayAuthorized()` | conduitd (relayToken registration, approval push) + the iOS app (APNs token registration) |
| **2 — per-session capability** | `relayToken` (32 random bytes, base64url, per session) | `POST /approval/decision`, `GET /decisions` | `relaySessionAuthorized()` (constant-time) | the iOS app (decision POST) + conduitd (decision poll) |

Tier 1 exists so conduitd can **bootstrap** a session's `relayToken` before any per-session capability exists. Tier 2 is the actual anti-spoofing control: even a party holding the shared secret cannot forge a cross-session decision without the session's `relayToken`, which is delivered only over the already-authenticated DaemonChannel.
...
### 3a. conduitd → app: relayToken delivery (DaemonChannel handshake)
- **RPC method:** `conduit.device.register` (the existing app→conduitd session handshake; the app already sends this with its session info).
...
```
  ...
  The app must read `result.relayToken` from the `conduit.device.register` reply and store it for the session. Treat it as a secret.
...
- The `sessionId` in this body **must equal** the `sessionID` the app passed to `conduit.device.register` (that is the key the relayToken was registered under). Missing/wrong/unknown token → **401**, no side effects.

### 3c. conduitd → backend: decision poll (already wired here)
- `GET {backend}/decisions?sessionId=<id>`
- **Header:** `Authorization: Bearer <relayToken>`

### 3d. conduitd → backend: relayToken registration (already wired here)
...
- `POST {backend}/register` with body `{ "sessionId": ..., "deviceToken": ... }` and the control-plane secret. `/register` now upserts `deviceToken` and `relayToken` **independently**, so the app's APNs registration and conduitd's relayToken registration can arrive in any order without clobbering each other.
...
### conduitd (`daemon/conduitd/`)
- **`relay_token.go` (new)** — `generateRelayToken()`: 32 bytes `crypto/rand`, base64url no-pad.
- **`server.go`**
  - `server` gains `relayToken` (guarded by `deviceMu`).
  - `conduit.device.register` mints (or reuses, across reconnects) the per-session relayToken, registers it with the backend via `postRelayRegistration` (async, control-plane authed), starts the poller with it, and returns `{ "relayToken": ... }`.
...
## 5. conduitd poll-path approveAlways / audit gap — closed
...
ok  	conduit/push-backend
$ go test -race ./...
ok  	conduit/push-backend

$ cd daemon/conduitd && go vet ./... && go test ./...
ok  	conduit/conduitd
ok  	conduit/conduitd/policy
$ go test -race ./...
ok  	conduit/conduitd
ok  	conduit/conduitd/policy
```

vet clean, race clean, both modules. Test runs (incl. subtests): push-backend **90**, conduitd **59**. New/changed coverage: correct token → 2xx; missing/wrong/unknown token → 401 (POST and poll); rejected posts leak nothing; relayToken TTL eviction → de-authorizes; control-plane secret on `/register`; independent apns/relay upsert; relayToken minted + returned in the handshake + registered with the backend; poll sends `Bearer`; poll-path `approveAlways` → policy + audit (and plain approve → audit only, no policy).

## 7. Residual risk / flags

1. **Registration trust = control-plane secret (no TOFU).** `/register` is last-writer-wins (per the "keep it simple" follow-up): a holder of `APPROVAL_RELAY_SECRET` could overwrite a session's relayToken. Mitigations: the secret is deployment-scoped and never client-distributed; the original "anyone with a leaked sessionId" bypass is closed. Hardening option (deferred): TOFU — record the relayToken on first registration and 409 on change for a live session — at the cost of breaking re-registration after a conduitd restart that re-mints. Flagged, not implemented.
