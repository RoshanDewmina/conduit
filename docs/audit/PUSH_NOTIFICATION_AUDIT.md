...
```
iOS App                      Push Backend                  lancerd Daemon
───────                      ────────────                  ──────────────
...
9. User taps Approve ───► LancerNotificationDelegate             │
                         routes via NotificationCenter             │
                         → ApprovalActionBuffer                   │
                         → ApprovalRelay.decide()                 │
10. Decision sent back to lancerd via WebSocket                  │
```
...
| 2 | `registerForRemoteNotifications()` | ✅ Implemented | `LancerApp.swift:112` |
| 3 | `didRegisterForRemoteNotificationsWithDeviceToken` callback | ✅ Implemented | `LancerApp.swift:118-128` |
| 4 | Token POSTed to push backend `/register` | ✅ Implemented | `Notifications.swift:230-239` |
| 4a | `pushBackendURL` from Info.plist (`LANCER_PUSH_BACKEND_URL`) | ✅ Configured | `LancerApp.swift:27`, `Info.plist:35` |
| 5 | lancerd sends approval event to push backend | ✅ Backend handles | `push-backend/main.go:228-260` (`handleApproval`) |
...
| 9 | `LancerNotificationDelegate.didReceive` routes action | ✅ Implemented | `LancerApp.swift:168-217` |
| 9a | Cold-launch buffer (MAJOR-6) | ✅ Implemented | `Notifications.swift:48-71` (`ApprovalActionBuffer`) |
| 9b | Background push refresh | ✅ Implemented | `LancerApp.swift:137-149` (`didReceiveRemoteNotification`) |
...
|---|---|---|---|
| `aps-environment` | `production` | ✅ Set | `Lancer.entitlements:6` |
...
| `APNS_BUNDLE_ID` | `push-backend/main.go:300` | Must be `dev.lancer.mobile` |
...
2. lancerd sends approval/run-complete events to the push backend.
...
|---|---|---|
| Simulator (no APNs) | ✅ | `didFailToRegisterForRemoteNotificationsWithError` logs silently; `pushBackendURL` empty check at `LancerApp.swift:120` skips registration |
...
| Empty `LANCER_PUSH_BACKEND_URL` | ✅ | Guard at `LancerApp.swift:120` skips registration entirely |