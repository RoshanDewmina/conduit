# APP_AUDIT.md — Conduit Codebase Audit
...
Conduit is organized as a Swift Package (`ConduitKit`) with multiple modules plus a thin Xcode app target. Key architectural rules enforced throughout:
...
|---|---|---|
| **ConduitCore** | Shared models + protocols | `ConduitDProtocol.swift`, `Approval.swift`, `Snippet.swift`, `SessionSummary` |
...
- **ConduitWatch** — watchOS multi-tab app (Inbox, Activity, Session, Snippets tabs)
- **ConduitLiveActivityWidget** — Dynamic Island + Lock Screen Live Activity
- **ConduitWatchWidget** — watchOS complication

### Go Layer

- **`daemon/conduitd`** — hook ingest, approve/reject routing (pure-Go hook gateway)
...
| watchOS multi-tab app | ✅ Complete | Inbox, Activity, Session, Snippets tabs in ConduitWatch |
...
| conduitd MVP | ✅ Complete | `agent-hook` command, risk mapping, auto-approve fallback |
...
| **APNs alert loop** | ❌ Gap | `conduitd` never POSTs to `push-backend`; alert loop unclosed — agent waits, phone stays silent |
| **Token-routing** | ❌ Gap | iOS registers push token with `identifierForVendor`; conduitd keys approvals by agent session → device↔session mismatch |
| **Structured tool_use** | ❌ Gap | `conduit-hook.sh:18-25` flattens `tool_input` to 500-char string; no `toolName`/`toolUseID`/`input` fields in wire protocol |
...
`conduit-hook.sh:18-25` reads Claude Code's hook payload and flattens the entire `tool_input` into a single 500-character truncated string. **No structured fields** — `toolName`, `toolUseID`, `sessionId`, structured `input` — flow through the wire protocol. This means:
...
2. **Approval wire protocol is structurally lossy** — `conduit-hook.sh` flat-string problem (see §3.3). Until fixed, approval cards show truncated text; DiffKit is unused on real data; edit-before-run is impossible.

3. **APNs alert loop is open** — `conduitd` never POSTs to `push-backend`; token-routing mismatch (`identifierForVendor` vs. agent session key). The #1 user complaint ("I missed the moment") is unresolved while this gap exists.
...
6. **conduitd is not open-sourced** — harder for security-conscious developers to trust and self-host. Open-sourcing is the highest distribution leverage action available (see ROADMAP.md §4).
...
### conduit-hook.sh (the lossy layer)
File: `daemon/conduit-hook.sh` lines 18-25.
Current behavior: reads stdin JSON from Claude Code's PreToolUse hook; extracts only `tool_name` and flattens `tool_input` into a 500-char string; POSTs to conduitd. Structured fields are lost before conduitd ever sees them.

### push-backend billing.go
File: `daemon/push-backend/billing.go` (~482 lines).
Status: Stripe billing wired; APNs send path exists in the binary but conduitd never calls the `/approval` endpoint. The push-backend is deployed and reachable (HTTPS via Caddy auto-TLS per commit 293bcd3b); the gap is on the conduitd side.
