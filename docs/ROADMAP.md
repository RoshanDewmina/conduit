...
2. **Pure-Go hooks near-term; Node Agent-SDK bridge deferred.** The hook gateway (`conduit-hook.sh` + `conduitd`) is the near-term approval path. The Claude Code Agent SDK `canUseTool` callback (edit-before-run, richer semantics) requires a Node sidecar; deferred to managed-compute phase.
...
| **3** | Structured approval cards — the approval spine | Structured tool_use wire protocol (2.1a); always-approve persistence + edit-before-run (2.1b) | Round-trip test passes (hook → conduitd → iOS → approve → unblocks agent); `.approvedAlways` persisted and re-applied; edit-before-run returns edited input to agent |
| **4** | Multi-agent fleet (as craft, not headline) | FleetStore; jump-to-unread; fleet-wide Inbox (2.2) | ≥2 independent agent slots; restore-after-reconnect test passes; no new Swift 6 warnings |
| **5** | Security / self-host + distribution | E2E/on-prem hardening; open-source conduitd bridge (4.6); enterprise pricing | Bridge packaged + documented; security posture written; first enterprise customer or partnership |
...
|---|---|---|---|---|---|
| **1.1** | Flip `DeviceTesting.entitlements` → `Conduit.entitlements`; enable CloudKit + Push in `project.yml` | S | High | **EXTERNAL: paid Apple Developer account ($99/yr)** | 0+1 |
| **1.2** | Close APNs alert loop: conduitd POSTs pending approvals to push-backend `/approval`; fix token-routing mismatch (`identifierForVendor` → agent-session key); wire `didReceiveRemoteNotification` in iOS app | M | High | dep 1.1 | **2** |
...
The features that make Conduit worth choosing over first-party Anthropic Remote Control and free OSS alternatives.

| ID | Item | Size | Risk | Deps | Stage |
|---|---|---|---|---|---|
| **2.1a** | **Structured tool_use wire protocol** — un-flatten `conduit-hook.sh`; add `toolName`/`toolUseID`/`sessionId`/structured `input` to `ApprovalEvent` (Go) + `ApprovalPendingParams`/`Approval` (Swift) + DB migration | L | Med | — | **3 (SPINE)** |
| **2.1b** | **Real "Allow always" + edit-before-run** — persist always-rules in conduitd (`rules.go`); stop discarding `.approvedAlways` in `DaemonChannel.swift:52`; decision payload carries edited input; "Edit & run" + per-rule "Always allow" in InboxView | L | Med | dep 2.1a | **3** |
...
| **4.6** | **Open-source conduitd bridge** — highest distribution leverage; Happy (21.6k★) + CloudCLI (11.6k★) model | M | Low | **Stage 5** |
...
Distribution is the biggest execution risk — Conduit will be the best-built and potentially the least-known app in a crowded field.

### 5.1 Open-Source the conduitd Bridge (Stage 5)
...
Anthropic Remote Control requires Anthropic's infrastructure. Conduit's conduitd bridge runs on the developer's own host. This is a structural advantage for the compliance/enterprise segment and is not replicable by first-party tools.
...
| OSS rivals (Happy/cmux) improve faster than Conduit | Med | High | Native iOS quality (Live Activities, DI, Watch) is structurally harder for OSS to match |
...
| conduitd trust gap (closed source) | Med | Med | Open-source in Stage 5 mitigates; interim: publish architecture docs |
