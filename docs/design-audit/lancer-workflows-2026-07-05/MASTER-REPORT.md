...
- **Cursor shell is production navigation**: the app uses `CursorAppShell` under `AppFeature/CursorStyle/` — three roots (Home / Workspaces / Settings), search via the Search sheet, threads under Workspaces. Legacy sidebar / `LancerSidebarView` was **deleted** (2026-07-06). `ARCHITECTURE.md` §4.1 is authoritative; this report's §3 sidebar claims (2026-07-05) are historical.
