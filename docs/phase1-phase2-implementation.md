...
This document covers the implementation of Phase 0 (platform upgrade), Phase 1 (make SSH work end-to-end), and Phase 2 (session survival) for Lancer — the mobile cmux.

Prior to this work, Lancer had a well-architected codebase (~8,600 lines, 80+ files) but was functionally at early M1. SSH shell opening was stubbed, sessions were never auto-connected, and reconnection logic was unimplemented.
...
- Agent Mode uses approval-based command execution — maps to Lancer's inbox concept
- Warp Drive offers parameterized workflows in YAML — inspiration for snippets
- **No mobile Warp app exists** — Lancer fills this gap
...
- Snippets **lack parameterized variables** — opportunity for Lancer to ship first
...
- cmux is local-only; Lancer is the mobile equivalent for remote work
...
**Files modified:**
- `Packages/LancerKit/Package.swift` — swift-tools-version 6.0 → 6.2, `.iOS(.v17)` → `.iOS(.v26)`, `.macOS(.v14)` → `.macOS(.v15)`
...
- Added 15-second connection timeout using task-group racing pattern
  ...
  - Throws `LancerError.timeout` if connect hangs
