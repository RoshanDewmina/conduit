#!/usr/bin/env bash
# OpenCode PreToolUse hook for Lancer iOS approval cards.
#
...
#   cp docs/opencode-lancer-hook.sh ~/.config/opencode/hooks/lancer-hook.sh
#   chmod 700 ~/.config/opencode/hooks/lancer-hook.sh
#
# Configure:
...
# Only gate tool calls for lancerd-dispatched opencode runs.
# The lancerd realLauncher sets LANCER_GATE=1 for every opencode process it
...
if [ ! -x "$LANCERD" ]; then
  printf '%s\n' "Lancer daemon not installed; auto-approving OpenCode tool call." >&2
  exit 0
fi
...
printf '%s\n' "Blocked by Lancer: OpenCode action was rejected on the iOS app or timed out." >&2
exit 2
