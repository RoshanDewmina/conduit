# Conduit — Live Loop Runbook (governed approvals + notifications, end-to-end)

> **Purpose.** A self-contained, step-by-step procedure to bring up and prove Conduit's core loop:
> a real agent (Claude Code / Codex / OpenCode / Kimi) on a host → `conduitd` policy → the phone's
...
```
agent PreToolUse hook ─▶ ~/.conduit/conduitd.sock ─▶ policyEngine.evaluate()
   │                                                      │
   │                              allow → audit+return    │ ask → blast-radius + queue
   │                              deny  → audit+return     ▼
   │                                          ┌─ SSH: conduitd serve relays to phone
...
**Fail-closed:** if the resident daemon is down, mutating hook kinds (`command`/`patch`/`fileWrite`/…) **hold (exit 1)**; only read-only kinds may fail-open and only with `CONDUIT_HOOK_READONLY_FAIL_OPEN=1`.

**Key paths:** binary `~/.conduit/bin/conduitd` · socket `~/.conduit/conduitd.sock` · queue `~/.conduit/queue.json` · policy `~/.conduit/policy.yaml` (+ repo-local `<cwd>/.conduit/policy.yaml`) · audit `~/.conduit/audit.log` (0600 JSONL). Hooks: `docs/conduit-hook.sh` (Claude), `docs/codex-conduit-hook.sh`, `docs/opencode-conduit-hook.sh`.

---

## PHASE 1 — Build everything from current source

> The shipped prebuilt `conduitd-darwin-arm64` is **stale** (Swift 0.1.0, no policy engine). Always rebuild conduitd from Go.

```bash
cd /Users/roshansilva/Documents/command-center

# 1a. conduitd (Go) — the ONLY canonical daemon
cd daemon/conduitd && go build -o conduitd . && go test ./... && cd ../..

# 1b. ConduitKit engines (fast inner loop; does NOT compile #if os(iOS) UI)
cd Packages/ConduitKit && swift build && cd ../..
```
Then the **authoritative iOS app-target build** (catches strict-concurrency breaks SPM misses). Prefer XcodeBuildMCP:
`session_set_defaults` → project `Conduit.xcodeproj`, scheme `Conduit`, sim `iPhone 17 Pro` → `build_sim`.
CLI fallback:
```bash
xcodebuild -project Conduit.xcodeproj -scheme Conduit \
  -destination 'platform=iOS Simulator,name=iPhone 17 Pro' -derivedDataPath /tmp/conduit-dd build
```
...
```bash
security add-generic-password -s 'conduit-localhost-ssh' -a "$USER" -w 'YOUR_LOGIN_PW' -U
```
...
```bash
mkdir -p /tmp/conduit-e2e-workspace/.claude
cp docs/conduit-hook.sh ~/.claude/hooks/conduit-hook.sh && chmod 700 ~/.claude/hooks/conduit-hook.sh
cat > /tmp/conduit-e2e-workspace/.claude/settings.json <<'JSON'
{ "hooks": { "PreToolUse": [ { "hooks": [ { "type": "command", "command": "~/.claude/hooks/conduit-hook.sh" } ] } ] } }
JSON
```
...
```bash
cd daemon/conduitd && CONDUITD_BINARY=./conduitd ../../scripts/validation/resident-bridge-smoke.sh
```

🛑 **CHECKPOINT 2:** socket exists, `conduitd version` prints a Go build (not 0.1.0), smoke script PASSes. With the daemon **stopped**, a `fileWrite` hook should print *“holding mutating action”* and exit 1 (fail-closed).
...
```
What it does: builds the app, installs to the booted sim, launches the **session harness** (`CONDUIT_GALLERY=session`) with `CONDUIT_TEST_AUTOCMD=claude` so a block forms without typing, screenshots `before-approval`, waits for you to tap **Approve**, screenshots `after-approval`.
...
```bash
cd /tmp/conduit-e2e-workspace && claude 'write the text "live-loop" to ./tc.txt'
```

🛑 **CHECKPOINT 3 (the core loop):**
- A pending **Inbox card** appears with real cwd / file / matched-rule / blast-radius.
- Tap **Approve** → within seconds `~/.conduit/audit.log` gets an `approve` entry (`tail -1 ~/.conduit/audit.log`) and `./tc.txt` is created.
...
2. When it finishes a turn, use the **follow-up** composer to send another prompt. This calls `continueRun` → `continueArgv` (Claude `--continue -p`, Codex `exec resume --last`, Kimi `--continue`, OpenCode equivalent), **with a new Conduit `runId`** but the vendor's session continuity underneath. Gates re-run.
...
`APPROVAL_RELAY_SECRET`, `APNS_KEY_ID=L8LVU9X82W`, `APNS_TEAM_ID=39HM2X8GS6`, `APNS_BUNDLE_ID=dev.conduit.mobile`, `APNS_KEY_PATH=/secrets/apns.p8`. `.p8` source: `~/Downloads/Personal-Docs/AuthKey_L8LVU9X82W.p8` (never commit).

### 5b. Relay pairing (works on sim — proves the decision-relay path, not APNs delivery)
Pair the phone to the relay in **Settings → Connection** (the app generates a pairing code; the host side connects with it). In DEBUG you can headless-pair the sim with `SIMCTL_CHILD_CONDUIT_RELAY_CODE=<6-digit code>`. With relay paired and **no SSH session**, an escalated approval should still reach the Inbox (delivered via `conduitE2EApprovalReceived` → mapped into the active `InboxViewModel`).
...
1. Launch Conduit once, accept notifications → device registers its APNs token (`ConduitApp.didRegisterForRemoteNotificationsWithDeviceToken` → `Notifications.registerDeviceToken`). Confirm the backend received the token for this `sessionId`.
...
5. Tap **Approve** on the lock screen → the decision routes through `ConduitNotificationDelegate` → `.conduitApprovalAction` (and, on cold launch, `ApprovalActionBuffer` drain) → agent unblocks **without opening the app to the foreground first**.
...
|---|---|---|
| **A.** Session header “Offline”, no card over SSH | daemon channel not attached (the pass-1 bug class) | `AppRoot.startSession` channel arm; `SessionViewModel.trustHostKey()` must call `onReconnected?()`; confirm a `conduitd serve` process persists (`pgrep -fl "conduitd serve"`). Heavy interactive zsh can wedge the unified shell. |
| **B.** Card appears, Approve does nothing, agent times out (120s→deny) | decision not delivered / ID mismatch | conduitd `approvalStore` case-insensitive lookup (fixed); over relay, `ApprovalRelay.forwardDecisionOnly`; check `sessionId` parity (reg vs decision POST). |
| **C.** Hook auto-allows everything | hook not gating / daemon governance missing | confirm policy `default: ask`; confirm conduitd is the **Go** build (not 0.1.0); confirm hook is the project-scoped one. |
...
```
You are executing Conduit's live-loop bring-up. Repo: /Users/roshansilva/Documents/command-center.

FIRST: invoke the `conduit-context-onboarding` skill, then read docs/LIVE_LOOP_RUNBOOK.md in full and
ARCHITECTURE.md §0.1 + §4.1. The app home is a sidebar/New Chat shell, not a tab bar. conduitd is the
Go source under daemon/conduitd (the shipped prebuilt binary is stale — rebuild it).
...
RULES:
- Verify with the authoritative builds: `go test ./...` from daemon/conduitd, and the XcodeBuildMCP
  app-target build (NOT just `swift build`). Use the `conduit-verification-gate` skill to choose checks.
- Never gate the owner's own Claude session — keep the test hook project-scoped to /tmp/conduit-e2e-workspace.
