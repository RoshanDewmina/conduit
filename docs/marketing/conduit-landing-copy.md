# Conduit — landing page copy

> Attach this with the design prompt. It maps Conduit's copy onto the **same section order as the reference site** (stoopper.framer.website). Use this text verbatim. Keep the reference's visual style, colors, type, and motion — only the words and the phone screens change.
>
...
3. **Your code never leaves your machine** — A small bridge (`conduitd`) runs on your host and enforces the policy you set. Conduit never gets your source or your credentials.
...
1. **Install the bridge** — On your host: `cd daemon/conduitd && go build -o conduitd . && ./conduitd install`. It runs in the background and survives reboots.
2. **Point your agent's hook** — One line for each agent — Claude Code, Codex, or opencode — so Conduit sees what they're about to do.
...
## 6. Why Conduit (3 value props — replaces "Why Choose Us")
1. **Cross-vendor, not single-vendor** — Claude's and OpenAI's mobile control each govern only their own agent. Conduit is the one policy, approval, and audit layer across all of them.
...
3. **Enforcement you can trust** — Conduit's hook returns a hard deny that holds **even under `--dangerously-skip-permissions`**. Default is fail-closed: if the bridge isn't reachable, mutating actions hold rather than auto-run.

## 7. "Proof" band (replaces the 6 fake testimonials — HONEST equivalents only)
**Section headline:** What Conduit guarantees
...
| | **Free** | **Conduit Pro** *(recommended)* | **Team & Self-host** |
|---|---|---|---|
...
- **Footnote:** Conduit Pro is a one-time purchase, not a subscription. Team/self-host pricing isn't set yet — tell us what you need.
...
- **What does it cost?** Free to use; a one-time Conduit Pro unlock is $14.99. Team/self-host is **[PLANNED]**.
