...
Visual source: `/Users/roshansilva/Downloads/Claude design conversation continuation/Conduit App.dc.html`.
...
|---|---|---|
| First-run onboarding | `CONDUIT_GALLERY=onboarding-redesign` | Sand canvas, terracotta editorial hero, three-step copy, policy controls, and 44-point actions reviewed. The XCUITest navigation and policy-selection flow passes. |
| Command Home | `CONDUIT_GALLERY=home` | Default Command Home layout reviewed with attention band, New Chat action, machine entry point, and populated recent work. |
| Sidebar | `CONDUIT_GALLERY=shell-sidebar` | Home, Inbox, Machines, Settings, search, and recent-thread hierarchy reviewed. |
| Inbox | `CONDUIT_GALLERY=shell-inbox` | Populated high/medium/low-risk approval cards, non-color risk labels, and approve/deny affordances reviewed. |
| Machines | `CONDUIT_GALLERY=shell-fleet` | Saved-machine detail surface and reconnect affordances reviewed; this replaces Fleet as a root destination. |
| Settings | `CONDUIT_GALLERY=shell-settings` | Nested settings groups, relay pairing, security, and reset state reviewed. |
| Live session error state | `CONDUIT_GALLERY=session` | The dark terminal context and authentication error surface reviewed without a live host credential. |
| iPad shell | `CONDUIT_UITEST_RESEED=1`, `CONDUIT_DESTINATION=home` | `NavigationSplitView` renders a persistent sidebar and Command Home detail; no tab bar is present. |
...
- Motion uses `ConduitMotion`; shell and onboarding transitions resolve to no animation under Reduce Motion. Repeating DesignSystem indicators already short-circuit under Reduce Motion.
...
Focused 25-second ETTrace 1.1.0 simulator captures were collected on iPhone 17 Pro/iOS 27 after temporarily linking the profiler (the target wiring was removed before handoff). Processed flamegraphs are retained locally at `/tmp/codex-ios-ettrace-conduit/run-{home,inbox,session}/output_259.json`.
