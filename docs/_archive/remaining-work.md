# Lancer — Remaining Work Before Production
...
> "v0.1.0 lancerd on GCP" notes describe the **stale Swift** daemon; the canonical
> daemon is the Go source (policy engine + resident `daemon`). For current state use
> **`docs/PUBLISH_READINESS_CHECKLIST.md`** + **`docs/LANCER_PROJECT_DOSSIER.md`**
...
- ✅ Phase 7 — OSC 133 missing-marker fallback, byte-pattern interactive-
  CLI hint, os.signpost telemetry, full `lancerGlassChrome` adoption,
...
- DaemonChannel (lancerd JSON-RPC over SSH) wired ✅
- ApprovalIngest (ingest daemon events into ApprovalRepository) wired ✅
- LiveInboxViewModel with real Allow/Reject → lancerd response ✅
- Codex hook: `docs/codex-lancer-hook.sh` + `docs/codex-hooks.json` ✅
- lancerd `agent-hook` command with risk mapping, patch support, auto-approve fallback ✅
...
- LancerWatchWidget (inbox count widget) ✅
- App group entitlement (`group.dev.lancer.mobile`) ✅
...
- Privacy manifest (Lancer/PrivacyInfo.xcprivacy) ✅ — declares optional APNs device identifier, no tracking
...
- `lancer://` URL scheme registered in Info.plist + project.yml ✅
...
### Quality
- 106/106 SwiftPM tests passing in 24 suites (verified 2026-05-28 via `swift test` on `Packages/LancerKit`) ✅
- 116/116 iOS simulator tests passing on iPhone 17 Pro (verified 2026-05-28 via XcodeBuildMCP `test_sim`, scheme `LancerKitTests`) ✅
...
- `lancer://billing/complete` deep link fires URL handler ✅
- lancerd v0.1.0 installed on GCP (35.201.3.231), hook wired ✅
- lancerd auto-approve fallback verified (exits 0 when socket absent) ✅
...
- [ ] Create app: Bundle ID `dev.lancer.mobile`
- [ ] Add IAP: `dev.lancer.mobile.pro` | Non-Consumable | $14.99 | "Lancer Pro"
- [ ] Enable CloudKit container: `iCloud.dev.lancer.mobile`
...
1. Product → Archive (scheme: Lancer, destination: Any iOS Device)
...
2. Run `codex`, then `/hooks` → trust `~/.codex/hooks/lancer-hook.sh`
3. Open Lancer iOS → connect to `35.201.3.231` → stay on Inbox tab
4. Give Codex a file-write task: `create /tmp/lancer-test.txt with "hello"`
5. **Reject once** → verify `/tmp/lancer-test.txt` not created
6. **Allow once** → verify `/tmp/lancer-test.txt` contains `"hello"`
7. Check `~/.lancer/codex-hook-events.jsonl` for the two event records

See `docs/SERVER.md` for full instructions.

### Stripe billing backend

1. Create recurring Stripe Prices for monthly and annual Lancer Pro.
...
4. Set `pushBackendURL` in `Lancer/LancerApp.swift` to the Fly.io URL

### CloudKit sync (needs paid account for container activation)
SyncKit architecture is implemented. The container `iCloud.dev.lancer.mobile` needs to be
