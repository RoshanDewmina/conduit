# Lancer — Project Dossier (State of the Project)
...
- **Now (pivot #1 — resident daemon):** a **helper program runs on the host** (`lancerd`). It owns approval state and a Unix socket that **outlives any single SSH session**. The phone attaches/detaches; the daemon keeps enforcing policy and queuing approvals in between. The phone became a **decision tool**, not a pipe. (`docs/lancerd-resident.md`, `docs/FRONTEND_DESIGN_BRIEF.md:10`)
- **Now (pivot #2 — hosted cloud):** a **control-plane backend** (`push-backend`) lets Lancer *also* run agents on cloud infra it provisions (Fly/GCP/Lightsail), metered by **prepaid credits** with **Stripe** billing, **quotas**, **schedules**, and **team orgs**. This turns "steer agents on YOUR machine" into "…or on machines we run for you," a different (compute-resale) business.
...
| **3. Resident daemon + policy/audit** | May–Jun 2026 | `lancerd` Go daemon installed on host; hook ingest for Claude Code / Codex / opencode; risk scoring; **policy engine (deny→ask→allow, fail-closed default=ask)**; **audit log**; allow-always; blast-radius; offline `queue.json`; InboxView + ApprovalIngest round-trip. |
...
```
   ┌──────────────────────────────────────────────────────────────────────┐
   │ iOS app  (Packages/LancerKit — SwiftUI, 22 SPM targets)             │
   │  • SSH/block terminal   • approval inbox   • fleet/billing   • policy │
   └───────────┬───────────────────────────────┬──────────────────────────┘
               │ SSH stdio (lancerd serve)     │ HTTPS (Bearer)
               ▼                                ▼
   ┌───────────────────────────┐    ┌─────────────────────────────────────┐
   │ lancerd  (Go)            │    │ push-backend  (Go control plane)     │
...
- `CreditBalance` (prepaidUSD/overageUSD/allowOverage) — `LancerCore/CreditBalance.swift`.
- `FleetSlotManager<T>` (≤3 slots, platform-independent) — `LancerCore/FleetSlotManager.swift`.
...
- `BridgeSessionActions` (struct of closures; decouples policy/dispatch UI from the live channel) — `LancerCore/BridgeSessionActions.swift`.
- `AppRoot` / `AppEnvironment` (tabs: hosts·inbox·library·settings; sheets; routes notifications + fleet slots; **debug gallery** via `LANCER_GALLERY`) — `AppFeature/AppRoot.swift`.

**Navigation reality:** real tabs are `hosts / inbox / library / settings`, terminal-as-home. Billing and the bridge policy editor are wired into **real** Settings navigation (gated by `showPaidSurfaces` + cloud entitlement / live SSH connection), not gallery-only. The `LANCER_GALLERY` env routes render mock UI for visual review (see `CLAUDE.md`).

### 3.2 `lancerd` — the resident daemon (Go, `daemon/lancerd/`)
...
**Resident model** (`resident.go`, `server.go`): `daemon` listens on `~/.lancer/lancerd.sock`. Two connection types by first byte: **hook** (raw JSON `ApprovalEvent`) vs **attach** (`{"op":"attach"}`, then 4-byte length-prefixed JSON-RPC frames). On attach, pending approvals **drain** to the phone; decisions relay back. State survives SSH drops.
...
- **Policy** = ranked effects **deny > ask > allow**, default **ask** (fail-closed). Sources: repo-local `<cwd>/.lancer/policy.yaml` (walked up), global `~/.lancer/policy.yaml`, and **`~/.lancer/policy-always.yaml`** override (allow-always). Rule fields: id/effect/agent/tool/kind/match-glob/cwd-glob/min-max-risk (`policy/types.go`).
...
- **Audit log** `~/.lancer/audit.log` (JSONL 0600, secret-redacted): `audit.go`.
...
**Fail-closed** (`hook.go`): if the daemon is unreachable, mutating kinds (command/patch/fileWrite/fileDelete/network/credential/browser/callMCP/unknown) **exit 1** (hold); read-only (read/grep/list/search) fail-open only if `LANCER_HOOK_READONLY_FAIL_OPEN=1`; critical risk always holds.

**JSON-RPC to the phone** (`server.go`): `ping`, `agent.approval.response`, `agent.audit.tail`, `agent.policy.get/reload/set`, `agent.status` (per-vendor usage), `lancer.device.register` (push backend URL + session), `agent.dispatch` / `agent.cancel`, `agent.schedule.add/list/remove`. Plus an unsolicited `agent.approval.pending` notification.

**State dir** `~/.lancer/` (override `LANCER_STATE_DIR`): `lancerd.sock`, `queue.json` (0600), `policy.yaml`, `policy-always.yaml`, `schedules.json`, `audit.log`, `bin/lancerd`. Smoke test: `scripts/validation/resident-bridge-smoke.sh`. Tests: 47 Go test funcs.
...
- **Approval (the daily heartbeat):** agent PreToolUse hook → `lancerd.sock` → policy eval → if *ask*: blast-radius + queue + phone notify + `push-backend /approval` → APNs → phone decides (`allow|allowAlways|editAndRun|deny`) → relayed back → hook exits 0/1 → agent proceeds/blocks. Offline → queued in `queue.json`, drains on next attach.
...
- ❌→✅ **"`.approvedAlways` collapsed to `approve` in `DaemonChannel.swift`; rule never persisted"** — `DaemonChannel.swift:111` sends `"approveAlways"`; lancerd `appendAllowAlways` writes it to `policy-always.yaml`.
- ❌→✅ **"lancerd never POSTs approvals to push-backend (APNs loop open)"** — `server.go:532-533` `go s.postApprovalPush()` → `{PushBackendURL}/approval` (`server.go:613`). (APNs key + paid account + deployed backend all exist now; only a real-device delivery smoke test remains — see §10/§11.)
...
Four concepts: **Agents** (vendor·model·cwd·status), **the Bridge** (`lancerd`: connected vs running-without-you), **Approvals** (allow / allow-always / edit-then-run / deny), **Policy** (the rules that mean *most things never reach the human*). Emotional target: **calm confidence** — "I'm in control precisely because I'm not asked about everything." Context of use: **interrupt-driven, one-handed, glancing**, occasionally deep-focus.
...
- **5** security / self-host + **open-source `lancerd`** + enterprise pricing.
...
- **Glass chrome:** all translucent surfaces via `View.lancerGlassChrome(...)` (single source).
...
4. **The "loop control plane" framing.** The owner's memory frames Lancer as *the mobile surface for agent loops* (loop status, escalation rules, eval-as-first-class, loop-template marketplace). The current prototype encodes some of this (loop-A timeline, report-A proof card) but the backend has no first-class "loop" object or `lancer_loop_start`/`lancer_step_complete` MCP tools yet. **Is "loop engineering" the headline, and should the data model add a Loop primitive?**
...
7. **Open-sourcing `lancerd`.** Highest-leverage distribution move per the roadmap, but it exposes the policy/bridge plumbing. **When, and with what license/boundary** (bridge OSS, app proprietary)?
...
- iOS: `Packages/LancerKit/Sources/{AppFeature,LancerCore,SSHTransport,AgentKit,SettingsFeature,SyncKit,SessionFeature,InboxFeature,DesignSystem,…}`; tests `Packages/LancerKit/Tests/LancerKitTests/`.
- Daemons: `daemon/lancerd/` (resident), `daemon/push-backend/` (control plane), `daemon/agent-runner/` (cloud runner).
- Hooks: `docs/lancer-hook.sh` (Claude), `docs/codex-lancer-hook.sh`, `docs/opencode-lancer-hook.sh`.
- UX prototype: `docs/lancer-ui-prototype/` (Next.js). Sales brief: `docs/lancer-brief/index.html`.
...
**Strategy/current:** `FRONTEND_DESIGN_BRIEF.md`, `PRODUCT_RESEARCH.md`, `ROADMAP.md`, `PRODUCTION_READINESS_PLAN.md`, this dossier. **Architecture:** `agent-contract.md`, `block-terminal-implementation.md`, `lancerd-resident.md`, `hosted-agents-phase2.md`, `docs/_archive/cloud-execution-engine-plan.md` (treat its "missing" list as partly **stale** per §4.1). **Ops:** `ship-gate-owner-steps.md`, `resident-daemon-owner-steps.md`, `validation-playbook.md`, `cloud-run-production-cutover.md`. **Caveat:** `docs/_archive/APP_AUDIT.md` + `docs/_archive/current-state-audit.md` are point-in-time and **partly superseded** by §4.
...
---
*End of dossier. Maintained as the single source of truth for "what Lancer is and where it stands." When a §4 gap closes or a strategic fork (§9) is decided, update here first.*
