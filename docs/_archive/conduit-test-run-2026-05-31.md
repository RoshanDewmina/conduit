...
## Environment
- **Simulator:** iPhone 17 Pro (booted), Conduit `dev.conduit.mobile`, light + dark.
...
**Symptom:** On a live SSH session to a real zsh host, the Conduit shell-integration bootstrap
(`__conduit_preexec`/`__conduit_prompt_command`/the `if [ -n "${ZSH_VERSION-}" ]…fi` block) is
...
## Deviations / risks
- Cleared 5 stale tmux sessions on `127.0.0.1` (May-30 QA leftovers: idle prompts + 1 idle ConduitKit
...
- `ls` block → `✓ exit 0`, 1.07s, **output present** (no-output symptom gone); no `then>`/`function
  quote>` prompts; no echoed `__conduit_*` text; no rogue raw-terminal escalation. (`/tmp/ws10-verify-ls2.png`)
