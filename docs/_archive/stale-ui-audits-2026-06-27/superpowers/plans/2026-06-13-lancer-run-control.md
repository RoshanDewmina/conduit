# Lancer Run-Control (Two-Way Control v1) Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add phone→agent run-control — **stop (kill), pause/resume, and set-budget** — over the existing lancerd channel, surfaced in a Run Detail screen reachable from Fleet.

**Architecture:** lancerd already tracks launched runs in a `dispatcher` (`daemon/lancerd/dispatch.go`) and exposes `agent.cancel`. We extend the process handle to support SIGSTOP/SIGCONT, add `dispatcher.pause/resume/setBudget` with continuous budget enforcement via the existing spend feed, and add three RPCs (`agent.pause`, `agent.resume`, `agent.budget.set`). On iOS, `DaemonChannel` gains three methods mirroring `cancelRun`; a `RunControlStore` (depending on a `RunControlling` protocol for testability) drives a `RunDetailView` with confirm + haptics per the consistency rules.

**Tech Stack:** Go (lancerd, stdlib + `syscall`), Swift 6 / SwiftUI (LancerKit), XCTest, `go test`.
...
**lancerd (Go):**
- `daemon/lancerd/dispatch.go` — MODIFY: `procHandle` type, richer `launchFunc`, `pause/resume/setBudget/enforce`, `BudgetUSD` on `dispatchRun`.
- `daemon/lancerd/dispatch_test.go` — MODIFY: fake handle, pause/resume/budget tests.
- `daemon/lancerd/server.go` — MODIFY: three RPC cases in the dispatch switch.
- `daemon/lancerd/server_test.go` — MODIFY: RPC round-trip tests.

**LancerKit (Swift):**
- `Packages/LancerKit/Sources/SSHTransport/DaemonChannel.swift` — MODIFY: `pauseRun/resumeRun/setRunBudget`.
- `Packages/LancerKit/Sources/AppFeature/RunControlStore.swift` — CREATE: `RunControlling` protocol + `RunControlStore`.
- `Packages/LancerKit/Tests/AppFeatureTests/RunControlStoreTests.swift` — CREATE: fake-backed tests.
- `Packages/LancerKit/Sources/AppFeature/RunDetailView.swift` — CREATE: the screen (mirrors board `AgentRunDetailScreen`).
- `Packages/LancerKit/Sources/AppFeature/FleetView.swift` — MODIFY: navigate a row → `RunDetailView`.

---

## Task 1: Process handle with pause/resume (lancerd)
...
Run: `cd daemon/lancerd && go test -run TestProcHandlePauseResumeRecorded ./...`
...
Run: `cd daemon/lancerd && go build ./... && go test ./...`
...
git commit -m "feat(lancerd): process handle with pause/resume + run-control state"
```

---

## Task 2: Continuous budget enforcement (lancerd)
...
Run: `cd daemon/lancerd && go test -run 'TestSetBudget|TestSpendUpdate' ./...`
...
```bash
git add daemon/lancerd/dispatch.go daemon/lancerd/dispatch_test.go
git commit -m "feat(lancerd): continuous per-run budget enforcement + setBudget"
```

---

## Task 3: Run-control RPCs (lancerd server)
...
Run: `cd daemon/lancerd && go test -run TestRunControlRPCs ./...`
...
Run: `cd daemon/lancerd && go test -run TestRunControlRPCs ./... && go vet ./...`
...
git commit -m "feat(lancerd): agent.pause / agent.resume / agent.budget.set RPCs"
```
...
**Files:**
- Create: `Packages/LancerKit/Sources/AppFeature/RunControlStore.swift`
- Test: `Packages/LancerKit/Tests/AppFeatureTests/RunControlStoreTests.swift`
...
Run: `cd Packages/LancerKit && swift test --filter RunControlStoreTests`
...
Run: `cd Packages/LancerKit && swift test --filter RunControlStoreTests && swift build`
Expected: PASS, build succeeds.

- [ ] **Step 6: Commit**

```bash
git add Packages/LancerKit/Sources/AppFeature/RunControlStore.swift Packages/LancerKit/Tests/AppFeatureTests/RunControlStoreTests.swift Packages/LancerKit/Sources/SSHTransport/DaemonChannel.swift
...
**Files:**
- Create: `Packages/LancerKit/Sources/AppFeature/RunDetailView.swift`
- Modify: `Packages/LancerKit/Sources/AppFeature/FleetView.swift`

> Visual reference: the board's `AgentRunDetailScreen` (`docs/audit/migration-board/cc-screens-3.jsx`).
> Obey `docs/audit/LANCER_UI_CONSISTENCY_RULES.md`: destructive-left Stop, single footer, haptics.
...
Run: `cd Packages/LancerKit && swift test --filter testControlAvailability`
...
Run via XcodeBuildMCP `build_sim` (scheme Lancer) — per CLAUDE.md, the app build catches strict-concurrency issues `swift build` won't. Expected: build succeeds.

- [ ] **Step 8: Commit**

```bash
git add Packages/LancerKit/Sources/AppFeature/RunDetailView.swift Packages/LancerKit/Sources/AppFeature/RunControlStore.swift Packages/LancerKit/Sources/AppFeature/FleetView.swift Packages/LancerKit/Tests/AppFeatureTests/RunControlStoreTests.swift
git commit -m "feat(ios): Run Detail screen with stop/pause/resume/budget + haptics"
```

---

## Task 7: Audit trail for run-control (lancerd)
...
Run: `cd daemon/lancerd && go test -run TestPauseIsAudited ./...`
...
Run: `cd daemon/lancerd && go test ./... && go vet ./...`
Expected: PASS

- [ ] **Step 5: Commit**

```bash
git add daemon/lancerd/dispatch.go daemon/lancerd/dispatch_test.go daemon/lancerd/server.go
git commit -m "feat(lancerd): audit run-control actions (pause/resume/stop/budget)"
```
...
**Risk note:** SIGSTOP/SIGCONT are Unix-only; lancerd targets macOS/Linux hosts, so this is fine. Budget enforcement is event-driven (on `setSpentUSD` from the spend feed) — continuous polling/metering is a T1 concern, not duplicated here.
