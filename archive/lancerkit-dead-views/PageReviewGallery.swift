...
// MARK: - Combined pages review (launch with SIMCTL_CHILD_LANCER_GALLERY=pages)
...
private struct SettingsReviewContent: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 20) {
            ...
            settingsSection("ABOUT LANCER") {
                aboutSettingsRow(icon: "server.rack", title: "BYO host",
                    detail: "Connect to any SSH server you own or rent. Lancer does not provision or manage your infrastructure.")
                settingsDivider
                ...
                aboutSettingsRow(icon: "person.badge.minus", title: "No account required",
                    detail: "No Lancer login. No subscription. All session data stays on-device.")
            }
        }
        .padding(.horizontal, 16)
        .padding(.vertical, 16)
...
// MARK: - Isolated About section gallery (LANCER_GALLERY=settings-about)
    ...
    var body: some View {
        ZStack {
            ...
            VStack(alignment: .leading, spacing: 0) {
                ...
                Text("Keys are stored on-device (Keychain) and sent directly to the provider over TLS — never to Lancer servers.")
                    .font(.dsSansPt(12))
                    .foregroundStyle(t.text3)
                    .padding(.horizontal, 20)
                    .padding(.top, 12)
...
// MARK: - Library gallery (LANCER_GALLERY=library)
...
// MARK: - AddHost gallery (LANCER_GALLERY=addhost)
//
...
// MARK: - PersistentStatusBar gallery (LANCER_GALLERY=statusbar)
