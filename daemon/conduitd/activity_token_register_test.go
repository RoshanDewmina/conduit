...
// TestActivityTokenRegister verifies that conduit.device.register.activity
...
func TestActivityTokenRegister(t *testing.T) {
	...
	s.handleMessage(&rpcMessage{JSONRPC: "2.0", ID: 1, Method: "conduit.device.register.activity", Params: params})
	...
	case <-time.After(2 * time.Second):
		t.Fatal("conduitd did not POST to /register-activity-token")
	}
...
func TestActivityTokenRegisterInvalidParams(t *testing.T) {
	...
	s.handleMessage(&rpcMessage{JSONRPC: "2.0", ID: 2, Method: "conduit.device.register.activity", Params: params})
