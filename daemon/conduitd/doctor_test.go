...
func TestCheckConduitDir(t *testing.T) {
	dir := t.TempDir()
	conduit := filepath.Join(dir, ".conduit")
	if err := os.MkdirAll(conduit, 0700); err != nil {
		t.Fatal(err)
	}
	if r := checkConduitDir(conduit); r.status != statusOK {
		t.Fatalf("present 0700 dir: status = %v (%s)", r.status, r.message)
	}

	if r := checkConduitDir(filepath.Join(dir, "missing")); r.status != statusFail || !r.critical {
	...
	if r := checkConduitDir(loose); r.status != statusWarn {
...
func TestCheckInstalledBinary(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(binDir, "conduitd"), []byte("x"), 0755); err != nil {
		t.Fatal(err)
	}
