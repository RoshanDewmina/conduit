...
// git.go — host-side git/PR operations exposed as conduitd RPCs.
//
...
// direct-SSH GitClient which bypasses conduitd governance.
//
...
// surface a clear, actionable message pointing at the conduit doctor gh check —
...
func (s *server) openPR(p shipParams, branch string) (string, error) {
	...
	if err != nil {
		...
		case strings.Contains(low, "executable file not found"),
			...
			return "", errors.New("GitHub CLI (gh) is not installed on the host — run `conduit doctor` and install gh to open PRs")
		case strings.Contains(low, "auth") || strings.Contains(low, "gh_token") || strings.Contains(low, "login"):
			return "", errors.New("GitHub CLI (gh) is not authenticated on the host — run `gh auth login` (or set GH_TOKEN); see `conduit doctor`")
		default:
...
// ── CI proxy (bridge push-backend webhook ring buffer → conduitd) ───────────
...
// ConduitCore.CIEvent decodes. Type values: "pr" | "check_run" | "status" map to
