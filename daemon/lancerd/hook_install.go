...
// exact shape Lancer expects is documented in docs/claude-settings-hook.json
// (and docs/opencode-hooks.json for OpenCode).

// claudeHookCommand is the command string Lancer registers in the PreToolUse
...
#
# Installed by ` + "`lancerd install`" + ` and wired into ~/.claude/settings.json.
# Canonical copy: docs/lancer-hook.sh
...
// claudeHookScriptPath returns ~/.claude/hooks/lancer-hook.sh for the given home.
func claudeHookScriptPath(home string) string {
	return filepath.Join(home, ".claude", "hooks", "lancer-hook.sh")
}
...
// mergeClaudeHookEntry returns settings JSON with the Lancer PreToolUse hook
...
// wireClaudeHookSettings idempotently merges the Lancer PreToolUse hook into
...
// claudeHookWired reports whether the Lancer PreToolUse hook is registered in
...
func atomicWriteFile(path string, data []byte, perm os.FileMode) error {
	...
	tmp, err := os.CreateTemp(dir, ".lancer-settings-*")
	if err != nil {
		return err
	}
	tmpName := tmp.Name()
