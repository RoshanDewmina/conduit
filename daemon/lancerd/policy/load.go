...
// LancerDir returns ~/.lancer (or $home/.lancer).
func LancerDir(home string) string {
	return filepath.Join(home, ".lancer")
}

func GlobalPolicyPath(home string) string {
	return filepath.Join(LancerDir(home), GlobalPolicyFile)
}
...
// LoadRepoPolicy walks up from cwd for .lancer/policy.yaml.
func LoadRepoPolicy(cwd string) (Document, string, error) {
	...
	for {
		candidate := filepath.Join(dir, ".lancer", GlobalPolicyFile)
...
// AppendAllowRule appends an allow rule to ~/.lancer/policy-always.yaml (deduped).
