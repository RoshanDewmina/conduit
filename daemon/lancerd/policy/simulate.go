...
// auditEntry mirrors the JSONL format in ~/.lancer/audit.log.
...
// LoadAuditEntries reads the last periodDays worth of audit entries from ~/.lancer/audit.log.
func LoadAuditEntries(home string, periodDays int) ([]auditEntry, error) {
	path := filepath.Join(home, ".lancer", "audit.log")
