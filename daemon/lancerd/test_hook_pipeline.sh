#!/usr/bin/env bash
# test_hook_pipeline.sh — end-to-end shell test for the lancer-hook.sh pipeline.
#
# Run from the repo root:
#   bash daemon/lancerd/test_hook_pipeline.sh
#
...
#
# What it tests:
#   - lancer-hook.sh reads tool_name, tool_use_id, session_id, and tool_input
#     from stdin JSON (not from CLAUDE_* env vars)
#   - The mock lancerd stub receives the correct --tool-name, --tool-use-id,
#     --session-id, and --tool-input flags

set -u

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
HOOK_SCRIPT="$(cd "$SCRIPT_DIR/../.." && pwd)/docs/lancer-hook.sh"
SAMPLE_JSON="$SCRIPT_DIR/testdata/sample-pretooluse.json"
MOCK_LANCERD="$SCRIPT_DIR/testdata/mock-lancerd.sh"
...
[ -x "$MOCK_LANCERD" ]  || fail "mock lancerd not executable: $MOCK_LANCERD"

# Run the hook with mock lancerd; capture output
OUTPUT=$(LANCERD="$MOCK_LANCERD" bash "$HOOK_SCRIPT" < "$SAMPLE_JSON" 2>&1)
EXIT_CODE=$?

# The hook should exit 0 (mock lancerd always approves)
[ "$EXIT_CODE" -eq 0 ] || fail "hook exited $EXIT_CODE (expected 0). Output: $OUTPUT"
pass "hook exited 0"

# Check --tool-name
echo "$OUTPUT" | grep -q -- '--tool-name=Bash' \
  || fail "--tool-name=Bash not found in lancerd args. Got: $OUTPUT"
...
OUTPUT2=$(LANCERD="$MOCK_LANCERD" bash "$HOOK_SCRIPT" < "$SAMPLE_JSON" 2>&1)
