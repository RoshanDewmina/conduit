...
/// Sends an approval event to lancerd's stdio server from an agent hook.
/// Usage: lancerd agent-hook approval --agent claude-code --kind command --command "rm -rf /tmp" --cwd /home/user --risk medium
struct ApprovalHook {
    static func run(args: [String]) {
        ...
        guard let body = try? JSONSerialization.data(withJSONObject: event) else {
            fputs("lancerd agent-hook: failed to encode event\n", stderr)
            exit(1)
        }

        // Write framed event to stdout (for piping to lancerd)
        var length = UInt32(body.count).bigEndian
        let lengthData = withUnsafeBytes(of: &length) { Data($0) }
        FileHandle.standardOutput.write(lengthData)
        FileHandle.standardOutput.write(body)

        fputs("[lancerd] approval event sent: \(approvalID)\n", stderr)
    }
}
