# Lancer V1 — Relay redeploy + tester distribution: EXECUTED results (2026-06-20)
...
defect was a **stale running revision** (`lancer-push-00005`) predating the new
...
gcloud run deploy lancer-push --source . \
...
volume at `/secrets/apns.p8`; `APNS_BUNDLE_ID`). New live revision: **`lancer-push-00006-wbp`**.
...
**not** route to `lancer-push` and could not be repointed. Per owner decision,
...
`export LANCER_RELAY_URL="wss://conduit-push-y4wpy6zeva-ts.a.run.app"`  (base, no `/ws/relay`).
...
gcloud run services update lancer-push --region australia-southeast1 --project roshan-agent-f1c2466d \
...
**Root cause:** `RoshanDewmina/lancer` is **private**, so GitHub release assets `404`
...
- Published: `lancerd_{darwin,linux}_{arm64,amd64}`, `SHA256SUMS`, `install.sh`.
...
`~/.lancer/bin/lancerd` → runs, reports current build with `pair`/`daemon`/`serve`/
...
| Gate | Status |
|---|---|
| Relay `/health` 200 | ✅ new service `lancer-push-00006-wbp` |
| New device endpoints live (not 404) | ✅ on `.a.run.app` |
| `go test` × daemon modules (lancerd, push-backend, agent-runner) | ✅ |
...
code `652939`, `LANCER_RELAY_URL=wss://conduit-push-y4wpy6zeva-ts.a.run.app`):
1. **Found + fixed a 3rd P0:** relay rendezvous is an in-memory map (`websocket_relay.go` `hub.pairs`),
   ...
   --timeout 3600` (rev `lancer-push-00007-mh8`).
2. **Daemon → relay WS connect:** `e2e: connected to relay as daemon (code: 652939)` — Cloud Run
   WebSockets work for the relay.
3. **Phone ↔ daemon pairing:** launched the app with `SIMCTL_CHILD_LANCER_RELAY_{URL,CODE}` +
   `LANCER_PUSH_BACKEND_URL` → daemon logged `e2e: paired with phone (code: 652939)` at ~4s;
...
4. **Escalation queued + forwarded:** fired a `fileWrite` `agent-hook` → daemon held a real pending
   ...
   (needs `LANCER_SEED_DEMO=1`), so inbox content was real, not demo seed.
...
- **The gap:** with the app paired (daemon: `paired with phone`, no auto-allow, hook blocking), a forwarded
  ...
  handler (`AppRoot.swift:386` `lancerE2EApprovalReceived` → `activeInboxViewModel.approvals.insert`) would
...
2 sample pending cards even without `LANCER_SEED_DEMO=1` — the harness/test needs a clean-inbox seam to
