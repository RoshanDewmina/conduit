...
// ToolMapping describes a single MCP tool exposed by lancer-mcp.
// The Name is what the agent sees; Kind is passed to lancerd agent-hook --kind.
...
// Config is the top-level configuration file for lancer-mcp.
type Config struct {
...
func loadConfig(path string) *Config {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "lancer-mcp: cannot read config %s: %v\n", path, err)
		os.Exit(1)
	}
	var cfg Config
	if err := json.Unmarshal(data, &cfg); err != nil {
		fmt.Fprintf(os.Stderr, "lancer-mcp: invalid config: %v\n", err)
		os.Exit(1)
	}
	if cfg.Agent == "" {
		cfg.Agent = "unknown"
	...
	if cfg.SocketPath == "" {
		cfg.SocketPath = "~/.lancer/lancerd.sock"
	}
	return &cfg
}

func resolveConfigPath() string {
	if v := os.Getenv("LANCER_MCP_CONFIG"); v != "" {
		return v
	}
	if len(os.Args) > 1 {
		return os.Args[1]
	...
	if err != nil {
		fmt.Fprintln(os.Stderr, "lancer-mcp: cannot determine home directory")
		os.Exit(1)
	}
	return filepath.Join(home, ".lancer", "lancer-mcp.json")
}
