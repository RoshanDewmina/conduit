...
- **Deferred to V2 — full interactive terminal (owner decision 2026-06-30; Phase 1 re-wired 2026-07-16):** V1 originally deferred interactive terminal. Owner reversed that on 2026-07-12 (`docs/product/2026-07-12-orca-terminal-port-map.md`). **Phase 1 is now wired:** Trusted Machines → machine detail → Open Terminal, and thread ⋯ → "Open terminal at this cwd", presenting `LiveTerminalView` (SwiftTerm) over SSH with TOFU + password/ed25519. Phase 2 (daemon-owned PTY sessions) and Phase 3 (mobile input UX kit) remain future work. SFTP / port-forward / SOCKS preview stay unwired. V1's Work Thread still shows agent activity as a read-only log from daemon/relay events — the interactive shell is a separate drill-in, not the chat surface.
...
- **SSH + interactive terminal (Phase 1, 2026-07-16):** `LiveTerminalView` + `LiveTerminalModel` (SwiftTerm) re-wired into Workspaces via Trusted Machines machine detail and thread ⋯ menu. TOFU host-key sheet, password/ed25519 auth, optional `cd` startup command. Engine (`SessionViewModel` / `BlockRenderer` / `PTYBridge`) retained for Phase 2.
